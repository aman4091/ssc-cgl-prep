...
import {
  ...
  getChapterQuestions, addChapterQuestions, removeChapterQuestion, clearChapterQuestions,
} from "@/lib/grammar";
import {
  extractRules, extractPdfTextSmart, ocrImage, ruleQuiz,
  renderPdfToImages, generateQuizText, generateQuizChunked,
} from "@/lib/client-ai";
...
export default function ChapterPage() {
  ...
  const delQuestion = (idx) => { removeChapterQuestion(chapterId, idx); refresh(); };
              ...
              <div style={{ display: "grid", gap: 12 }}>
                {shown.map((q) => (
                  <PyqQuestionCard
                    ...
                    onDelete={() => delQuestion(questions.indexOf(q))}
                  />
                ))}
              </div>
            )}
