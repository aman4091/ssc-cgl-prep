// Mock-test marks tracker — the user records each sectional/full mock they take:
// per-section correct / wrong / total / time, plus a name and the day they added
// it. Everything derived (score, accuracy, attempted) is computed here so the UI
// just renders. Stored in localStorage (cgl.mockmarks) so it syncs like the rest.

import { storeGet, storeSet, storeRemove } from "./bigstore";

const KEY = "cgl.mockmarks";

// SSC CGL Tier-1 marking. Kept here so a change is one place.
export const PER_CORRECT = 2;
export const PER_WRONG = 0.5;

// 📈 Percentile — "mujhse peeche kitne pratishat log hain".
//
// Rank aur kul kitne, dono external site (Testbook / RBE) par saamne dikhte
// hain; unhi do se percentile ban jata hai. Score se percentile ka koi seedha
// rishta nahi hota — wo us din kaun-kaun baitha uspar nirbhar hai — isliye ise
// alag rakha hai aur sirf tab dikhate hain jab dono ginti daali ho.
export function percentileOf(rank, outOf) {
  const r = Number(rank), n = Number(outOf);
  if (!Number.isFinite(r) || !Number.isFinite(n) || r < 1 || n < 1 || r > n) return null;
  return Math.round(((n - r) / n) * 10000) / 100;
}

// The default section set for a FULL mock (SSC CGL Tier-1).
export const FULL_SECTIONS = ["General Intelligence & Reasoning", "General Awareness", "Quantitative Aptitude", "English Comprehension"];

// Menu buckets: one Full mock, plus a bucket per subject. Each mock is filed
// under one of these keys so a subject page shows only that subject's marks.
export const CATEGORIES = [
  { key: "full", label: "Full Mock", icon: "🎯", subject: null },
  { key: "reasoning", label: "Reasoning", icon: "🧠", subject: "Reasoning" },
  { key: "maths", label: "Maths", icon: "🧮", subject: "Quantitative Aptitude" },
  { key: "english", label: "English", icon: "📘", subject: "English" },
  { key: "gk", label: "GK / GS", icon: "🌍", subject: "General Awareness" },
];
export const categoryOf = (k) => CATEGORIES.find((c) => c.key === k) || CATEGORIES[0];

function read() {
  if (typeof window === "undefined") return [];
  try { const r = storeGet(KEY); return r ? JSON.parse(r) : []; }
  catch { return []; }
}
function write(v) { storeSet(KEY, JSON.stringify(v)); }

const num = (v) => { const n = Number(v); return Number.isFinite(n) && n >= 0 ? n : 0; };

// Per-section derived numbers.
export function sectionStats(s) {
  const correct = num(s.correct), wrong = num(s.wrong);
  const total = Math.max(num(s.total), correct + wrong);
  const attempted = correct + wrong;
  const score = correct * PER_CORRECT - wrong * PER_WRONG;
  const accuracy = attempted ? Math.round((correct / attempted) * 100) : 0;
  return { name: s.name || "Section", correct, wrong, total, attempted, unattempted: Math.max(0, total - attempted), score, accuracy, timeMin: num(s.timeMin) };
}

// Whole-mock totals across its sections.
export function mockTotals(rec) {
  const secs = (rec.sections || []).map(sectionStats);
  const t = secs.reduce((a, s) => ({
    correct: a.correct + s.correct, wrong: a.wrong + s.wrong, total: a.total + s.total,
    attempted: a.attempted + s.attempted, score: a.score + s.score, timeMin: a.timeMin + s.timeMin,
  }), { correct: 0, wrong: 0, total: 0, attempted: 0, score: 0, timeMin: 0 });
  t.accuracy = t.attempted ? Math.round((t.correct / t.attempted) * 100) : 0;
  t.sections = secs;
  return t;
}

// Newest attempt first (by date, then when it was added). Optionally filtered to
// one category bucket (full / reasoning / maths / english / gk).
export function getMocks(cat) {
  let all = read().slice();
  if (cat) all = all.filter((r) => (r.cat || "full") === cat);
  return all.sort((a, b) => (b.date || "").localeCompare(a.date || "") || (b.at || "").localeCompare(a.at || ""));
}

export function addMock({ name, cat, date, sections, rank, outOf, external }) {
  const rec = {
    id: `mk_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 7)}`,
    name: String(name || "").trim() || "Mock",
    cat: categoryOf(cat).key,
    date: date || new Date().toISOString().slice(0, 10),
    at: new Date().toISOString(),
    // Rank/kitne mein se — sirf tab jab dono theek se aaye hon. Adhoora rank
    // rakhne ka matlab nahi: percentile bina doosre ke banta hi nahi.
    rank: num(rank) || null,
    outOf: num(outOf) || null,
    // Bahar (Testbook/RBE) diya hua hai ya site ka apna — list mein 🌐 se pata
    // chal jata hai ki ye ginti kahan se aayi.
    external: !!external,
    sections: (sections || [])
      .filter((s) => (s.name && s.name.trim()) || num(s.correct) || num(s.wrong) || num(s.total))
      .map((s) => ({ name: String(s.name || "").trim(), correct: num(s.correct), wrong: num(s.wrong), total: num(s.total), timeMin: num(s.timeMin) })),
  };
  write([rec, ...read()]);
  return rec;
}

// Ek record ka percentile (rank/outOf pade ho to).
export const mockPercentile = (rec) => percentileOf(rec?.rank, rec?.outOf);

export function removeMock(id) { write(read().filter((r) => r.id !== id)); }
export function clearMocks() { storeRemove(KEY); }

// Headline numbers across all recorded mocks.
export function overallSummary() {
  const all = read();
  if (!all.length) return { count: 0, avgScore: 0, avgAccuracy: 0, best: 0 };
  const totals = all.map(mockTotals);
  const avgScore = Math.round((totals.reduce((a, t) => a + t.score, 0) / all.length) * 10) / 10;
  const avgAccuracy = Math.round(totals.reduce((a, t) => a + t.accuracy, 0) / all.length);
  const best = Math.max(...totals.map((t) => t.score));
  return { count: all.length, avgScore, avgAccuracy, best };
}
