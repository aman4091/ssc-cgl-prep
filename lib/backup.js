...
import { storeSet, storeSnapshot } from "./bigstore";
import { getAllHinglish } from "./noteshinglish";
import { LOCAL_ONLY } from "./syncmerge";
...
function collectLocalStorage() {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    // LOCAL_ONLY = live pomodoro state aur is device ka sync-base. Dono doosre
    // device par bekaar (ya nuksaandeh) hain, isliye backup mein bhi nahi jate.
    if (k && k.startsWith("cgl.") && !LOCAL_ONLY.has(k)) ls[k] = localStorage.getItem(k);
  }
  // Bade stores (quizzes, questions, wrongbook…) ab IndexedDB mein hain — upar
  // wala scan unhe nahi dekhta, isliye backup mein yahan se jodo. Warna backup
  // khaali jaisa banega aur restore par data gayab.
...
export async function importAll(obj, onProgress) {
  ...
  // storeSet: bade keys IndexedDB mein jaate hain, chhote localStorage mein —
  // restore par bhi quota nahi bharta.
  for (const k of Object.keys(ls)) { try { storeSet(k, ls[k]); } catch { /* quota */ } }
