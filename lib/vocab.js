...
// Entry = { w, at } — `at` add hone ka ISO time. Purane entries plain string
// the (date pata nahi) — unka at = "" aur wo "Purane words" group mein aate hain.
export function getNewWordEntries() {
  const list = read(VKEYS.newwords, [])
    .map((it) =>
      typeof it === "string"
        ? { w: it, at: "" }
        : { w: String(it.w || ""), at: String(it.at || "") }
    )
    .filter((e) => e.w);
