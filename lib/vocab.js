...
export function getNewWordEntries() {
  ...
  const normalized = raw
    .map((it) => {
      ...
      return { w: String(it.w || ""), at: String(it.at || "") };
    })
    .filter((e) => e.w);
...
export function addNewWord(word) {
  ...
  const has = (x) =>
    String(typeof x === "string" ? x : x?.w || "").toLowerCase() === w.toLowerCase();
...
export function removeNewWord(word) {
  ...
  const list = read(VKEYS.newwords, []).filter((x) => {
    const w = String(typeof x === "string" ? x : (x && x.w) || "");
  ...
  for (const k of Object.keys(mine)) {
    if (k.toLowerCase() === lw) { delete mine[k]; touched = true; }
  }
