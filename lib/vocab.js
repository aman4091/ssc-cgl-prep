...
export const VKEYS = {
  ...
  newwords: "cgl.vocab.newwords", // [words...] — user ke apne pakde hue words
};
...
export function getNewWords() {
  const list = read(VKEYS.newwords, []).map(String);
...
export function addNewWord(word) {
  ...
  if (list.some((x) => String(x).toLowerCase() === w.toLowerCase())) return;
  list.push(w);
