...
// Day quiz = ALL words learned so far (Day 1..day), all types. Words are picked
// via the coverage cycle so nothing repeats until every word has been quizzed.
export function buildDayQuiz(day) {
  const pool = getOws();
  const items = pickCycleItems(`vmixed:${day}`, getCumulativeItems(day), QUIZ_LIMIT);
...
// Same but only ONE type (OWS / idiom / vocab), cumulative + coverage cycle.
export function buildTypeQuiz(day, type) {
  const pool = getOws();
  const items = pickCycleItems(`vtype:${type}:${day}`, getCumulativeItems(day, type), QUIZ_LIMIT);
  const questions = items.map((it) => buildMcq(it, pool));
  return {
    id: makeId(),
