...
// 🗑️ Ek word HAR jagah se hatao — din wali list se bhi aur khud pakde
// hue se bhi. /vocab ki thaili dono ko milakar banti hai, isliye wahan ka
// delete dono taraf lagna chahiye; ek jagah rehne par word agli baar phir
// saamne aa jata.
export function removeWordEverywhere(word) {
  const lw = String(word || "").trim().toLowerCase();
  if (!lw) return;
  const left = getOws().filter((it) => String(it.word || "").trim().toLowerCase() !== lw);
  if (left.length !== getOws().length) saveOws(left);
  removeNewWord(word);
}
