...
function readAll() {
  ...
  try { return JSON.parse(storeGet(KEY) || "{}"); } catch { return {}; }
}
...
// Just the corrections for a given set of question ids (one bank), so the owner
// can export them and have me fold them into the committed file — the only way
// a local fix becomes permanent for everyone on a static site.
export function gkOverridesForIds(ids) {
  const all = readAll();
  const out = {};
...
export function saveGkOverride(id, patch) {
  ...
  try { storeSet(KEY, JSON.stringify(all)); } catch { /* quota */ }
  return all[id];
}

export function clearGkOverride(id) {
  const all = readAll();
  if (all[id]) { delete all[id]; try { storeSet(KEY, JSON.stringify(all)); } catch { /* ignore */ } }
}
