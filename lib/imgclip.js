...
export async function imageBlob(im) {
  if (im?.url) {
    // Pehle apna proxy (r2.dev CORS header bhejta hi nahi). Proxy kisi wajah se
    // na chale — jaise deployment par R2 env set na ho — to seedha bhi try kar
    // lo; custom domain wali images wahan se aa jayengi.
    for (const u of [`/api/r2/image?url=${encodeURIComponent(im.url)}`, im.url]) {
      try { const res = await fetch(u); if (res.ok) return await res.blob(); } catch { /* agla */ }
    }
    throw new Error("Image load nahi hui.");
  }
