...
export function keyFromPublicUrl(url) {
  ...
  // Sirf wahi do folders jo ye app khud likhti hai — wrong/ (screenshots) aur
  // ink/ (handwriting). Baaki bucket mein maths/reasoning ke question banks aur
  // notes ke scans pade hain; unhe koi delete request chhoo bhi na paye.
  return /^(wrong|ink)\//.test(key) ? key : null;
}
