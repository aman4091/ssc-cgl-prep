...
const CORE = [
  ...
  { type: "B", checkpoint: 2, slot: { t: "Maths + GS sectional → Checkpoint 2", list: ["Q", "GS"] }, ca: "Jan–Feb 2026 (sirf sports, awards, appointments)",
...
const FINAL = [
  { type: "A", rev: true, ca: "CA log revise (jo likha hai wahi)",
...
function blocksFor(day, m) {
  ...
  if (p.type === "A") {
    const mock = B("mock", 60, "mock", `FULL MOCK ${p.fm}/${totalMocks(m)}`, {
      must: true, kind: "mock", how: `${FULL_PAPER}. Phone bahar, koi pause nahi. Maths section: pehle 40 sec SCAN (GREEN/YELLOW/RED), fir green.${p.note ? " " + p.note : ""}`,
      href: "/mock-marks?cat=full", hrefLabel: "📊 Marks daalo",
    });
    const ana = B("ana", p.last ? 45 : 60, "mock", "Mock analysis — 6 step", { must: true, href: "/mission/analysis", hrefLabel: "🔍 Analysis" });
    return {
      pinned: [warm, mock, ana],
      seq: [rev, math, gs, brk(1), eng, tier2, voc, brk(2), gspyq, extraMath, caB, speed, err, night, next].filter(Boolean),
    };
  }
  if (p.type === "B") {
...
export const RULES = {
  mathSprint: { title: "Maths attempt 19 se kam → SPEED mode", detail: "Aage ke concept topics (Mensuration, Geometry, DI) hatake unki jagah 1-liner sprints. Speed drill +20 min. Round-1 ka 60-sec cap sakhti se.", adapt: true },
  ...
  gsPyqOnly: { title: "GS 18 se kam (Checkpoint 1)", detail: "Static reading band — GS block mein sirf PYQ solve + fact log.", adapt: true },
  gsAccept: { title: "GS 23 se kam (Checkpoint 2)", detail: "GS ka floor 22–25 maan lo. GS PYQ ka aadha time Maths sprint ko (zyada pakke marks). Tukka sirf 50-50 par.", adapt: true },
  engNoBlind: { title: "English mein har mock 5+ galat", detail: "Soch ke tukka band — 1 bhi option nahi kata to chhodo. Roz 30 error Q.", adapt: true },
...
export function evaluateCheckpoints(mocks, m = getMission()) {
  ...
  if (day >= c1 && s1.GS.n && s1.GS.score < TARGETS.cp1.GS.score) triggered.push("gsPyqOnly");
  if (day >= c2 && s2.GS.n && s2.GS.score < TARGETS.cp2.GS.score) triggered.push("gsAccept");
