...
// GS ka asli baseline 13 Sep ko mila: 25 attempt, 9 sahi = 10/50 (full mock
// mein bhi 9.5). Isliye GS floor 28 → 22, total 141 → 135.
export const FLOOR = { R: 41, GS: 22, Q: 34, E: 38, total: 135 };
export const STRETCH = { R: 45, GS: 28, Q: 39, E: 41, total: 153 };
export const FLOOR_NOTE = {
  R: "23 attempt, ~91% (full mock mein 31.5 aaya tha — 45 best-ever hai, target nahi)",
  GS: "25 attempt, ~58% (baseline 13 Sep: 10 = 9 sahi / 16 galat)",
...
const CORE = [
  { type: "B", slot: { t: "Maths self-test + GS BASELINE sectional (pehla asli GS)", list: ["DIAG", "GS"] }, ca: "Sep 2026",
    ...
    g: { t: "Static GK-1: 8 classical dances + folk, festivals → state, instruments → artists, stadiums", notes: "/pyq/war/static-gk", specs: [["war", "static-gk"]] } },
  { type: "A", note: "Baseline — jo aaye wahi sahi. Is number se hi aage ka hisaab banega.", ca: "Aug 2026",
    ...
    g: { t: "Static GK-2: awards, sports cups + terms, books → authors, important days, first-in-India, HQ", notes: "/pyq/war/static-gk", specs: [["war", "static-gk"]] } },
  { type: "B", slot: { t: "Maths + GS + Reasoning sectional", list: ["Q", "GS", "R"] }, ca: "Aug 2026",
    ...
    g: { t: "Polity-1: Articles (14, 19, 21, 21A, 32, 44, 51A, 72, 112, 123, 280, 324, 352/356/360, 368), 12 Schedules, Parts", notes: "/pyq/war/polity", specs: [["gk", "polity"], ["war", "polity"]] } },
  { type: "A", ca: "Jul 2026",
    ...
    g: { t: "Polity-2: Amendments (42, 44, 52, 61, 73, 74, 86, 101, 103), FR / DPSP / FD, bodies", notes: "/pyq/war/polity", specs: [["war", "polity"], ["gk", "polity"]] } },
  { type: "B", slot: { t: "Maths + GS + English sectional", list: ["Q", "GS", "E"] }, ca: "Jul 2026",
    ...
    g: { t: "Biology: vitamins, diseases + pathogen, human body, blood, cell", notes: "/pyq/war/biology", specs: [["war", "biology"]] } },
  { type: "A", ca: "Jun 2026",
    ...
    g: { t: "Chemistry: formulas, common names, alloys, pH, gases", notes: "/pyq/war/chemistry", specs: [["war", "chemistry"]] } },
  { type: "B", checkpoint: 1, slot: { t: "Maths + GS + Reasoning sectional → Checkpoint 1", list: ["Q", "GS", "R"] }, ca: "Jun 2026",
    ...
    g: { t: "Physics: SI units, instruments, inventors (numerical nahi)", notes: "/pyq/war/physics", specs: [["war", "physics"]] } },
  { type: "A", ca: "May 2026",
    ...
    g: { t: "Geography-1: rivers + tributaries, dams, passes, national parks", notes: "/pyq/war/geography", specs: [["war", "geography"]] } },
  { type: "B", slot: { t: "Maths + GS sectional", list: ["Q", "GS"] }, ca: "May 2026",
    ...
    g: { t: "Geography-2: soils, crops, Census 2011 + physical basics", notes: "/pyq/war/geography", specs: [["war", "geography"], ["war", "environment"]] } },
  { type: "B", slot: { t: "Maths + GS + English sectional", list: ["Q", "GS", "E"] }, ca: "Apr 2026",
    ...
    g: { t: "Modern History: 1857 → 1947 timeline, Congress sessions, Viceroys, movements + years", notes: "/pyq/war/modern-history", specs: [["war", "modern-history"]] } },
  { type: "A", ca: "Apr 2026",
    ...
    g: { t: "Economy — sirf PYQ, padhna nahi (AAKHRI naya topic)", notes: "/pyq/war/economics", specs: [["war", "economics"]] } },
  { type: "B", slot: { t: "Maths + GS sectional", list: ["Q", "GS"] }, ca: "Mar 2026",
    ...
    g: { t: "Ancient + Medieval — sirf PYQ, padhna nahi", notes: "/pyq/war/ancient-history", specs: [["war", "ancient-history"], ["war", "mediaeval-history"], ["gk", "ancient-history"]] } },
  { type: "B", slot: { t: "Maths + GS sectional", list: ["Q", "GS"] }, ca: "Mar 2026",
    ...
    g: { t: "Sabse kamzor GS cluster (fact log se) — Static GK + Polity + Science PYQ", notes: "/mission/facts", specs: [["war", "static-gk"], ["war", "polity"], ["war", "biology"]] } },
  ...
  { type: "A", checkpoint: 2, note: "Checkpoint 2 — isi mock ke saath.", ca: "Jan–Feb 2026 (sirf sports, awards, appointments)",
    ...
    g: { t: "Mixed PYQ recall: Static GK + Polity + Science + Geography", notes: "/mission/facts", specs: [["war", "static-gk"], ["war", "polity"], ["war", "biology"], ["war", "geography"]] } },
];
...
function blocksFor(day, m) {
  ...
  let mathHow = p.m && p.m.howX ? p.m.howX : "15 min formula/method (Brahmastra notes) → 25 PYQ timed (45 sec cap) → review.";
  ...
  // Block ka bantwara (Maths ka total time wahi): topic + AAKHIRI 20 MIN yellow
  // list — pichhle mock ke 40–75 sec wale Q ka short method, stopwatch ke saath
  // dobara (target <40). Pehle ye kaam 19:55 ke mistake-book bonus mein tha;
  // wo slot GS revision ko gaya, isliye yahan.
  const mathMin = p.type === "B" ? 110 : 90;
  const split = p.type === "B" ? "Bantwara: 70 min topic → 20 min sprint #2 → 20 min 🟡 YELLOW list." : "Bantwara: 70 min topic → 20 min 🟡 YELLOW list.";
  const math = p.m && B("math", mathMin, "maths", `Maths: ${mathT}`, {
    must: true, how: `${split} ${mathHow} 🟡 Yellow: pichhle mock ke 40–75 sec wale Q ka short method likho/padho, fir stopwatch ke saath dobara — target <40 sec.`,
    auto: mathSpecs.length ? { n: p.type === "B" ? 30 : 25, specs: mathSpecs } : null,
    href: "/notes/brahmastra", hrefLabel: "📐 Formulas", href2: "/mission/analysis", href2Label: "🟡 Yellow list",
    yellowList: true,
  });

  // ---- GS: PYQ → CLUSTER → REPEAT. Static reading BAND (GS baseline 10/50 —
  // 16 din mein syllabus padhne ka time nahi, sirf wahi yaad karna jo SSC
  // poochta hai). Ek galti = poora group ek line mein (8–15 facts).
  let gsT = p.g ? `GS PYQ + CLUSTER: ${p.g.t}` : "";
  let gsHow = "Notes NAHI. 50 PYQ (har Q max 10 sec, nahi pata → agla) → har galat/unsure ka CLUSTER (poora group EK line mein, 8–15 facts) → Fact log. DeepSeek answer ka 🧩 CLUSTER section seedha copy karo. Target aaj 25–30 naye cluster.";
  let gsAuto = p.g ? { n: 50, specs: p.g.specs } : null;
  if (adapt.gsPyqOnly && p.g) {
    gsT = `GS PYQ + CLUSTER (Static GK + Polity pe wapas): ${p.g.t}`;
    gsHow = "Checkpoint rule: GS nahi badh raha — Static GK + Polity (sabse list-wale) pe wapas. Fact log check karo: har entry CLUSTER hai ya single fact?";
    gsAuto = { n: 50, specs: [["war", "static-gk"], ["war", "polity"], ...p.g.specs] };
  }
  const gsLabel = !p.g ? "" : p.g.notes.startsWith("/pyq") ? "🎯 PYQ bank" : p.g.notes.startsWith("/notes") ? "📔 Notes" : "🧠 Fact log";
  const gs = p.g && B("gs", 90, "gs", gsT, { must: true, how: gsHow, auto: gsAuto, href: p.g.notes, hrefLabel: gsLabel, href2: "/mission/facts", href2Label: "🧠 Fact log" });

  // English: pehle 6 din grammar-heavy (12 rules finite hain aur 60% galtiyan
  // wahin — ek baar seekhe to pakke). D7 se grammar 30 + vocab SIRF 20: vocab
  // 7-8 Q ka infinite syllabus hai, GS 25 Q ka — bacha 40 min GS PYQ ko.
  const grammarPhase = day <= 6 && !p.ext;
  const engMin = grammarPhase ? 60 : 30, vocMin = grammarPhase ? 30 : 20;
  ...
  const voc = B("vocab", vocMin, "english", `PYQ vocab: ${vocab[1]}`, {
    must: true, how: (grammarPhase ? "30 PYQ words" : "20 PYQ words (baaki time GS ko)") + " + /vocab ka aaj ka batch. Naya/bhoola word → bookmark.",
    auto: { n: grammarPhase ? 30 : 20, specs: [["engbank", vocab[0]]] }, href: "/vocab", hrefLabel: "🔤 Vocab",
  });
  const gsx = !grammarPhase && B("gsx", 40, "gs", "GS PYQ set #3 (25 Q) + cluster — vocab ka time", {
    must: true, how: "Aaj ke topic ka ya kal ke weak cluster ka set. Har galat → cluster → Fact log.",
    auto: { n: 25, specs: p.g ? p.g.specs : [["war", "static-gk"], ["war", "polity"]] }, href: "/mission/facts", hrefLabel: "🧠 Fact log",
  });
  ...
  const gspyq = B("gspyq", adapt.gsAccept ? 30 : 60, "gs", adapt.gsAccept ? "GS PYQ: 1 set (25 Q) + cluster" : "GS PYQ mixed: 2 set × 25 Q + cluster", {
    must: true, how: "Mixed subjects (asli paper jaisa). Har Q max 10 sec → review → har galat/unsure ka CLUSTER → Fact log.",
    auto: { n: adapt.gsAccept ? 25 : 50, specs: gsSpecs }, href: "/mission/facts", hrefLabel: "🧠 Fact log",
  });
  ...
  // Pehle yahan Maths mistake-book (bonus) tha — ab GS cluster revision (must).
  // Maths ke yellow Q revision block mein 3 din baad khud aate hain.
  const err = B("gsrev", 35, "gs", "GS cluster revision — Fact log ke due (D+1 / D+3 / D+7), 40–50 cluster", {
    must: true, how: "Pehle khud yaad karo, fir 'Dikhao'. Bhool gaya dabane mein sharam nahi — wahi system hai.",
  ...
  if (p.type === "A") {
    ...
    return {
      ...
      seq: [rev, math, gs, brk(1), eng, tier2, voc, gsx, brk(2), gspyq, extraMath, caB, speed, reas, err, night, next].filter(Boolean),
    };
  }
  if (p.type === "B") {
  ...
  if (!p.noMock) {
    ...
    return {
      ...
      seq: [
        ...
        B("calog", 45, "gs", "CA log revise", { must: true, href: "/current-affairs", hrefLabel: "📰 CA" }),
...
// KRAM ZAROORI: re-solve SOLUTION DEKHNE SE PEHLE — answer pata ho to stopwatch
// jhootha (kam) time deta hai aur yellow bhi green dikhta hai.
export const ANALYSIS_STEPS = [
  "Marks /mock-marks mein daale (section-wise C/W/total/time + rank)",
  "SOLUTION DEKHNE SE PEHLE: jo Maths Q chhoote / jin par shak tha, stopwatch ke saath dobara — <40 sec 🟢 · 40–75 🟡 · >75 🔴. (Jo attempt kiye the unka time Testbook ke analysis mein har Q ka dikhta hai — wahi bucket.)",
  "Ab solution / answer key kholo → har galat/chhoda Q ko type do (W1–W4 / S1 / S2 / T1)",
  "🟡 ka short method solution se seekh ke likho · GS/English ka har galat fact/rule → Fact log",
...
// Maths block ke 20-min yellow slot ke liye: sabse naye analysis ki yellow list.
...
export const TARGETS = {
  cp1: { label: "D7", upto: 7, R: { score: 41, att: 23 }, Q: { score: 31, att: 19 }, E: { score: 36, wrongMax: 5 }, GS: { score: 16, att: 25 }, full: 120 },
  cp2: { label: "D14", upto: 14, R: { score: 41, att: 23 }, Q: { score: 33, att: 20 }, E: { score: 37, wrongMax: 4 }, GS: { score: 21, att: 25 }, full: 130 },
  exam: { label: "Exam floor", upto: null, R: { score: FLOOR.R, att: 23 }, Q: { score: FLOOR.Q, att: 20 }, E: { score: FLOOR.E, wrongMax: 3 }, GS: { score: FLOOR.GS, att: 25 }, full: FLOOR.total },
};

// 🌍 GS ka apna track (baseline 10) — har ~3 din ek GS sectional se naapo.
export const GS_TRACK = [
  { day: 4, score: 14, miss: "Cluster nahi ban rahe — fact log kholo, har entry check karo: single fact hai ya poora group?" },
  { day: 7, score: 16, miss: "Topic order galat ho sakta hai — Static GK + Polity pe wapas (Checkpoints par rule dikhega)." },
  { day: 10, score: 19, miss: "Revision peeche chhoot raha hai — walk + 08:00 block mein due cluster pehle." },
  { day: 13, score: 21, miss: "22 ke aas-paas accept karo, zyada mat dhakelo — bacha time Maths sprint ko." },
];
...
export const RULES = {
  ...
  gsPyqOnly: { title: "GS 16 se kam — Checkpoint 1 tak pichhle 3 mein se 2", detail: "Static reading pehle se band hai. Ab GS blocks Static GK + Polity (sabse list-wale) pe wapas, aur fact log ki har entry check: CLUSTER hai ya single fact?", adapt: true },
  gsAccept: { title: "GS 21 se kam — Checkpoint 2 tak pichhle 3 mein se 2", detail: "GS ~20–22 maan lo, zyada mat dhakelo. GS PYQ ka aadha time Maths sprint ko (zyada pakke marks).", adapt: true },
...
// GS track: har checkpoint din tak ka sabse naya GS sectional score.
