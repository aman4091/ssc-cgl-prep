...
//   GS         21.9 (att 25, acc 55%)    → SABSE BADA GAP, yahi phase 1 hai
...
export const FLOOR_NOTE = {
  ...
  GS: "25 attempt, 62% accuracy (abhi 55%) — poora khel cluster ka hai",
...
const DAYS = [
  ...
  { type: "B", slot: ["Q", "GS"],
    ...
    m: M("CI advanced + installments", [["sscmaths", "compound-interest-ci"], ["mathbank", "installment"]]),
  ...
  { type: "B", checkpoint: 2, slot: ["Q", "GS"],
    ...
    m: M("TSD: boats & streams, races, trains advanced", [["sscmaths", "boat-and-stream"], ["mathbank", "linear-circular-race"]]),
  ...
  { type: "B", slot: ["Q", "GS"],
    ...
    m: M("Trigonometry advanced (max/min)", [["sscmaths", "trigonometry"]]),
...
function blocksFor(day, m) {
  ...
  const rev = B("rev", 45, "rev", "Spaced revision — fact log ke due (D+1 / D+3 / D+7 / D+14)", {
    must: true, kind: "revision", href: "/mission/facts", hrefLabel: "🧠 Fact log",
    how: "'Dikhao' dabane se PEHLE khud yaad karo. 40–50 purane cluster.",
  });
  const calc = B("calc", 15, "maths", "Calc drill: tables 12–25, squares 1–30, fraction ↔ %", {
    must: true, href: "/calculation", hrefLabel: "🧮 Calc",
  });

  // ---- GS ke do bade block (CORE) — 50 PYQ + 15–18 cluster har ek.
  const gsHow = "Notes NAHI, sirf PYQ. Har Q max 10 sec — nahi pata to agla. Har galat/unsure ka CLUSTER: poora group EK line mein (6–12 facts), jaise 'Lata ke awards – Padma Bhushan 1969 · Phalke 1989 · Bharat Ratna 2001'. Target is block mein 15–18 naye cluster.";
  const gsLabel = !p.g ? "" : p.g.notes.startsWith("/pyq") ? "🎯 PYQ bank" : p.g.notes.startsWith("/notes") ? "📔 Notes" : "🧠 Fact log";
  const gs1 = p.g && B("gs1", 90, "gs", `GS PYQ + CLUSTER: ${p.g.t}`, {
    must: true, core: true, gate: true, how: gsHow,
    auto: { n: 50, specs: p.g.specs }, href: p.g.notes, hrefLabel: gsLabel,
    href2: "/mission/facts", href2Label: "🧠 Fact log",
  });
  const gs2 = p.g && B("gs2", ph === 1 ? 90 : 60, "gs", `GS PYQ + CLUSTER (set 2): ${p.g.t}`, {
    must: true, core: true, gate: true, how: gsHow,
  ...
  const gs3 = ph === 1 && B("gs3", 60, "gs", "GS mixed sectional (untimed) + cluster (8–10)", {
    must: true, how: "Mixed subjects, asli paper jaisa. Har galat → cluster → fact log.",
  ...
  const math = p.m && B("math", ph === 2 ? 120 : 60, "maths", `Maths: ${mathT}`, {
    must: true, core: true, how: mathHow,
  ...
  const voc = B("vocab", 20, "english", `PYQ vocab: ${vocab[1]}`, {
    must: true, how: "20 PYQ words + /vocab ka aaj ka batch. Naya/bhoola word → bookmark.",
  ...
  const speed = B("speed", 30, "maths", "Maths speed drill: 15 Q / 9 min sprint", {
    must: true, how: "Kisi Q pe 60 sec se zyada nahi. Yahi attempt 18.5 → 21 le jayega.",
  ...
  const reas = B("reas", 15, "reasoning", "Reasoning maintain: 10 series Q", {
    must: true, how: "Reasoning 44.9 hai — sirf girne mat do. Series mein sabse zyada galtiyan.",
  ...
  const night = B("night", 30, "rev", "🌙 Fact log revision + raat ka metric (3 number)", {
    must: true, core: true, gate: true, late: true, href: "/mission/facts", hrefLabel: "🧠 Fact log",
...
function taperBlocks(p, day, m) {
  const calc = B("calc", 15, "maths", "Calc drill (haath garam)", { must: true, href: "/calculation", hrefLabel: "🧮 Calc" });
  const facts = B("facts", 90, "gs", p.g ? p.g.t : "Revision: fact log", {
    must: true, core: true, gate: true, href: "/mission/facts", hrefLabel: "🧠 Fact log",
  ...
  const formula = B("formula", 60, "maths", "Maths formula sheet + mistake-book (sirf padhna)", {
    must: true, core: true, href: "/notes/brahmastra", hrefLabel: "📐 Formulas",
  });
  const rules = B("rules", 45, "english", "English rules one-pager + bookmarked vocab", {
    must: true, href: "/vocab/bookmarks", hrefLabel: "🔖 Bookmarks",
  });
  const calog = B("calog", 45, "gs", "CA log revise", { must: true, href: "/current-affairs", hrefLabel: "📰 CA" });
  const night = B("night", 30, "rev", "🌙 Fact log + raat ka metric", { must: true, core: true, gate: true, late: true, href: "/mission/facts", hrefLabel: "🧠 Fact log" });
  const admit = B("admit", 20, "life", "Admit card + ID ready, centre ka route/time", { must: true, href: "/mission/exam", hrefLabel: "🎯 Exam rules" });
