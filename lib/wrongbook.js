...
import { shedOldQuizzes, freeRegenerableSpace } from "./storage";
...
function write(v) {
  ...
  for (;;) {
    try { storeSet(KEY, json); return; }
      ...
      // Pehle quizzes/activity/feed jaise regenerable data hatao. Agar wo nahi
      // bacha to purane generated quizzes girao. Dono khatam ho gaye to genuine
      // quota full hai — throw karo.
      if (freeRegenerableSpace() > 0) continue;
      if (shedOldQuizzes()) continue;
      throw e;
    }
  }
}
...
export function getTombstones() {
  if (typeof window === "undefined") return {};
  try { return JSON.parse(storeGet(TOMB_KEY) || "{}") || {}; }
  catch { return {}; }
}
...
function writeTombstones(t) {
  ...
  try { storeSet(TOMB_KEY, JSON.stringify(kept)); }
...
// Delete kiye hue OVERLAY qid (q093…) — alag se, kyunki upar wale tombstone
// record ki apni id (wb_…) rakhte hain aur overlay us naam ko jaanta hi nahi.
//
// Iske bina site par 🗑️ ka koi matlab nahi tha: overlay ka resend_missing
// dekhta hai ki "ye maths question site ke paas nahi hai" aur 5 second baad
// use dobara bhej deta hai — question wapas aa jata tha. Ab ye list overlay
// ko bhi jaati hai, wahan se uski file hat jaati hai, aur wo dobara nahi
// bhejta.
const DELQ_KEY = "cgl.wrongbook.delqids";

export function getDeletedQids() {
  if (typeof window === "undefined") return [];
  try { const r = storeGet(DELQ_KEY); return r ? JSON.parse(r) || [] : []; }
  catch { return []; }
}

function noteDeletedQids(qids) {
  const add = [].concat(qids).filter(Boolean).map(String);
  if (!add.length) return;
  const seen = new Set(getDeletedQids());
  for (const q of add) seen.add(q);
  const list = [...seen].slice(-3000);
  try { storeSet(DELQ_KEY, JSON.stringify(list)); }
  catch { /* quota — agli delete par phir koshish */ }
}
...
// "Aakhri baar is question ko kab haath lagaya" — list ka poora kram isi par
// hai. Do mein se jo baad ka ho: record ka apna waqt, ya ✅ "Ho gaya" ka waqt.
//
// Ek hi hisaab do jagah chalta hai — Answers board par, aur OverlayInbox mein
// jo overlay ke panel ko kram bhejta hai. Isliye ye yahan rehta hai: dono
// jagah alag-alag likhne se kram kabhi na kabhi alag ho hi jata.
export function touchAt(rec, doneIso) {
  const a = String(rec?.at || "");
  const d = String(doneIso || "");
  return d > a ? d : a;
}

// Ek GHOOMTI hui katar: sabse upar wo jise sabse zyada der se haath nahi
// lagaya, sabse neeche wo jo abhi hua. Naya question bhi "abhi hua" hai,
// isliye wo nipte hue ke NEECHE lagta hai.
//
// `doneMap` = { id: kab-mark-hua } (lib/answersdone). Purane call jo Set bhejte
// the wo bhi chalte hain — waqt bas khaali reh jata hai.
export function displayOrder(items, doneMap) {
  const map = doneMap instanceof Set
    ? Object.fromEntries([...doneMap].map((id) => [id, ""]))
    : (doneMap || {});
  const qnum = (r) => {
  ...
  const cmp = (a, b) => {
    const ta = touchAt(a, map[a.id]);
    const tb = touchAt(b, map[b.id]);
...
// Overlay par question khola gaya par nipta nahi — use "abhi hua" bana do,
// taaki wo list mein sabse neeche chala jaye. Wahi ek cheez jo overlay ke
// panel ko aur is page ko ek saath hilati hai.
export function touchWrong(qid) {
  const all = read();
  const i = all.findIndex((r) => r.qid && r.qid === qid);
  if (i < 0) return false;
  all[i] = { ...all[i], at: new Date().toISOString() };
  write(all);
  return true;
}
...
export function addWrong({ subject, q = null, images = [], note = "", answer = "", qid = "", aiWant = false }) {
  const rec = {
    id: newId("wb"),
    subject,
    q,
    ...
    // 🌍 GS: iska answer overlay par DeepSeek likh raha hai (aiNotes mein
    // aayega) — tab tak card "ban raha hai" dikhata hai.
    ...(aiWant ? { aiWant: true } : {}),
  };
  write([rec, ...read()]);
  return rec;
...
// Overlay ka question number is record par chipka do.
//
// Ye un purane records ke liye hai jo site par HAATH SE paste hue the — unka
// koi qid nahi tha, isliye overlay ka panel unhe dikha hi nahi sakta tha, aur
// list ghoomne par ek din wo sabse upar aakar dono jagah ka pehla question
// alag kar dete. OverlayInbox unhe overlay ko bhej kar wahan ka qid le aata
// hai, aur wo yahan is se likh jata hai.
export function setQid(id, qid) {
  const q = String(qid || "").trim();
  if (!id || !q) return;
  const all = read();
  const i = all.findIndex((r) => r.id === id);
  if (i < 0 || all[i].qid) return;      // pehle se hai to chhedo mat
  all[i] = { ...all[i], qid: q };
  write(all);
}

// Overlay ke paas {record id: qid} ki poori list hai (jo question usne site
// se liye). Jis record ka qid yahan kho gaya ho, use ek hi write mein wapas.
//
// qid khota kaise tha: site ki ek purani tab apni purani copy (qid lagne se
// pehle ki) wapas likh deti thi. Tab wo question overlay se kat jata — panel
// par dikhta, site par "kahin nahi" (card par number hi nahi), aur overlay ka
// "Ho gaya" use dhoondh hi nahi pata tha. Jis qid ka record pehle se koi aur
// hai, wo nahi lagate — ek number do question par nahi hona chahiye.
// -> kitne theek hue
export function healQids(map) {
  if (!map || typeof map !== "object") return 0;
  const all = read();
  const taken = new Set(all.map((r) => r.qid).filter(Boolean));
  let n = 0;
  for (let i = 0; i < all.length; i++) {
    const r = all[i];
    const q = String(map[r.id] || "").trim();
    if (r.qid || !q || taken.has(q)) continue;
    all[i] = { ...r, qid: q };
    taken.add(q);
    n += 1;
  }
...
// `ans1At` / `ans2At` — pehla (detail) / dusra (detail2) Gemini answer KAB aaya
// (`ansAt` = dono mein jo baad ka). 2026-09-12 se har subject ka mukhya answer
// DeepSeek ka hai aur purane Gemini answer ab dikhte nahi (record mein pade
// hain, mite nahi). Sirf wo Gemini answer dikhta hai jo iske BAAD aaya —
// tumne 📥 se paste kiya, ✏️ se sudhara, ya overlay se copy hokar aaya. Har
// khaane ka apna nishaan isliye ki naya dusra answer paste karne par purana
// pehla (bina nishaan) wapas na dikhne lage.
...
export function setDetail(id, text) {
  ...
  if (i < 0) return;
  const t = stampNow();
  all[i] = { ...all[i], detail: String(text || "").trim(), ansAt: t, ans1At: t };
  write(all);
}

// 🤖 DeepSeek ka apne-aap wala answer (🌍 GS). Gemini wale answer (detail /
// detail2) se ALAG field — card par apne alag button ke peeche dikhta hai,
// aur haath se paste kiya answer isse kabhi nahi mitta.
export function setAiNotes(id, text) {
  const all = read();
  const i = all.findIndex((r) => r.id === id);
  if (i < 0) return false;
  const t = String(text || "").trim();
  if (all[i].aiNotes === t) return true;
  const next = { ...all[i], aiNotes: t };
  delete next.aiWant;
  all[i] = next;
  write(all);
  return true;
...
export function setDetail2(id, text) {
  ...
  if (i < 0) return;
  const t = stampNow();
  all[i] = { ...all[i], detail2: String(text || "").trim(), ansAt: t, ans2At: t };
  write(all);
}
...
// Naya (dikhne layak) Gemini answer — sirf nishaan wale khaane.
export const newGemini1 = (r) => (r?.ans1At ? r.detail || "" : "");
export const newGemini2 = (r) => (r?.ans2At ? r.detail2 || "" : "");
export const newGeminiAnswer = (r) => newGemini2(r) || newGemini1(r);
...
// padhne mein kachra hai. Overlay ka storage.clean_answer() bhi yahi safai
// karta tha. Yahan isliye ki /answers ka card aur /wrong/solve dono ek hi
// safai karein (pehle sirf /answers karta tha).
...
export function setShownDetail(id, text) {
  ...
  if (!rec) return;
  // Jo naya answer dikh raha hai wahi badlo (✏️ sirf naye par aata hai).
  if (newGemini2(rec) || (rec.detail2 && !newGemini1(rec))) setDetail2(id, text);
...
export async function removeWrong(id) {
  const rec = read().find((r) => r.id === id);
  markDeleted(id);
  if (rec?.qid) noteDeletedQids(rec.qid);
...
export async function clearWrong(subject) {
  ...
  noteDeletedQids(going.map((r) => r.qid).filter(Boolean));
