...
// Wrong-Book record -> ek option-less "question" jise FullscreenRunner attempt kar
// sake (image dikhega, options nahi; "Show answer" = answer diya). Timed hone par
// iski speed-key doneKeyFor = "id:wb:<id>" hoti hai.
export function wrongToQuestion(rec) {
  const url = (rec?.images || []).find((i) => i.url)?.url || "";
  return {
    id: "wb:" + rec.id,
    qImg: url,
    optImgs: [],
    solution: rec?.detail || rec?.answer || "",
    qText: rec?.note || "",
    wbId: rec?.id,
    optionless: true,
  };
...
export function getBuckets(subject) {
  ...
  try {
    for (const r of getWrongBook(subject) || []) {
      // Attempt ho chuka (timed record "id:wb:<id>" maujood) -> ab wo apne time
      // bucket mein hai, over2 reference mein nahi.
      if (seen.has("id:wb:" + r.id)) continue;
      out.over2.push({ key: "wb:" + r.id, kind: "wb", wb: r, at: r.at || "" });
    }
