...
// Full in-memory Hinglish (localStorage mirror + IndexedDB, merged) as a plain
// object. Cloud sync uses this so the DURABLE IndexedDB copy rides the snapshot
// even when the localStorage mirror couldn't be written (storage full) — which
// is why pasted Hinglish wasn't crossing devices.
export function getAllHinglish() { return { ...ensureCache() }; }
...
// Warm the cache (and kick IndexedDB hydration) as soon as this module loads on
// the client, so a cloud-sync push includes the full Hinglish without needing a
// notes page to be opened first this session.
if (typeof window !== "undefined") { try { ensureCache(); } catch { /* ignore */ } }
