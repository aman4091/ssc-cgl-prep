...
export async function caFromText(text, onProgress) {
  ...
  if (base.length) {
    const B = 8;
    const out = [];
    for (let i = 0; i < base.length; i += B) {
      const batch = base.slice(i, i + B);
      onProgress && onProgress("hinglish", Math.min(i + B, base.length), base.length);
      let hi = null;
      try { const r = await caHinglish(batch); hi = r && Array.isArray(r.items) ? r.items : null; }
      catch { hi = null; } // API down / quota → English fallback for this batch
      for (let j = 0; j < batch.length; j++) {
        const e = batch[j];
        const h = hi && hi[j];
        if (h && h.question && Array.isArray(h.options) && h.options.length >= 2) {
          const opts = h.options.map(String).slice(0, 4);
          const ans = Number.isInteger(h.answer) && h.answer >= 0 && h.answer < opts.length ? h.answer : e.answer;
          out.push({ question: String(h.question), options: opts, answer: ans, detail: String(h.detail || e.detail || "") });
        } else {
          out.push(e); // keep the reliable English version
        }
      }
    }
    return { questions: out, mode: "parsed" };
  }
  // Unknown format → let the model try to extract/generate.
