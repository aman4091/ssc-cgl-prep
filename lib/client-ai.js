...
import { parseCaMcqs } from "./caparse";
...
export function askAI({ question, imageText, mode, subject, customPrompt }) {
  const subj = baseSubject(subject);
  const extra = {};
  // Bulane wale ne apna prompt diya (🐋 DeepSeek button wahi prompt bhejta
  // hai jo ✨ Gemini copy karta hai) to wahi chalta hai — server ka apna
  // EXPLAIN_PROMPT tabhi jab yahan kuch na ho.
  if (customPrompt && customPrompt.trim()) extra.customPrompt = customPrompt.trim();
  if (mode === "shortcut" || mode === "ca") {
    ...
    if (mode === "shortcut") {
      ...
      if (!extra.customPrompt && cp && cp.trim()) extra.customPrompt = cp.trim();
    }
  }
...
// Ek maths/reasoning question -> uski 40-second wali trick (stylus page ke
// baayen taraf). Prompt server par hai (app/api/quick-trick).
export function quickTrick({ question, options, answer }) {
  return post("/api/quick-trick", { question, options, answer }); // -> { text }
}

// Notes ka ek page -> us page ke SSC wale facts (TOPIC / FACTS / CONFUSION).
// Prompt server par hai (app/api/notes-facts) — owner ka apna.
// 🧹 Sirf shakl sudharo — ek bhi fact badle bina (app/api/format-fact).
// Fact log ke us cluster ke liye jo ek chipke hue paragraph mein aaya ho.
export function formatFact(text) {
  return post("/api/format-fact", { text }); // -> { text }
}

// 💬 Site par kuch bhi select karke poochha gaya sawaal. `history` pichhli
// baatcheet hai, taaki "aur iska matlab?" jaisa chhota sawaal bhi chale.
export function askSelection({ selection, question, subject, history }) {
  return post("/api/selection-ask", { selection, question, subject: baseSubject(subject), history }); // -> { answer }
}

// ⚡ Sprint ka chhota jawab — ek question, teen line. Route apna model
// khud tay karta hai (deepseek-chat), isliye 100 call bhi sasta padta hai.
export function sprintAnswer({ question, options, correct, subject, kind }) {
  return post("/api/sprint-answer", { question, options, correct, subject: baseSubject(subject), kind }); // -> { answer }
}

// 🐋 One-liner notes ko sirf SHAKL do — Article number, saal, naam waale
// shabd **bold**. Ek bhi baat badle bina (app/api/format-oneliner).
export function formatOneLiner(text) {
  return post("/api/format-oneliner", { text }); // -> { text }
}

export function notesFacts(text, topic) {
  return post("/api/notes-facts", { text, topic }); // -> { text }
}
...
// Current-Affairs PDF text -> the questions IN it, rewritten faithfully in
// HINGLISH (question + options + a Hinglish `detail` answer), matching the CA
// display format. Returns { questions: [{ question, options[], answer, detail }] }.
export function caImport(text) {
  return post("/api/ca-import", { text }); // -> { questions }
}

// Rewrite one batch of parsed CA MCQs into Hinglish (server keeps answer index).
export function caHinglish(items) {
  return post("/api/ca-hinglish", { items }); // -> { items }
}

// THE main entry for the CA "+" importer. Reliable-first:
//   1) Parse the standard "Q.N) a-d / Correct Answer" format deterministically
//      (no AI — never returns empty on a PDF that clearly has questions).
//   2) Rewrite those in Hinglish in small batches; if a batch's AI call fails,
//      keep that batch in ENGLISH (questions are never lost).
//   3) If the PDF isn't in that format, fall back to full AI extraction.
// onProgress(phase, done, total). Returns { questions, mode: "parsed"|"ai" }.
export async function caFromText(text, onProgress) {
  const base = parseCaMcqs(text);
  if (base.length) {
    // Standard CA format parsed reliably (no AI, instant). Clear `detail` so the
    // CA detail page's existing auto-explainer writes a fresh HINGLISH answer for
    // each question progressively — exactly like the app already does. This keeps
    // import instant and can never return "no questions" on a valid CA PDF.
    onProgress && onProgress("parsed", base.length, base.length);
    return { questions: base.map((q) => ({ question: q.question, options: q.options, answer: q.answer, detail: "" })), mode: "parsed" };
  }
  // Unknown format → let the model try to extract/generate from the content.
  const r = await caImportChunked(text, (i, t) => onProgress && onProgress("ai", i, t));
  return { questions: r.questions, mode: "ai" };
}

// Chunked so a long CA PDF doesn't get clipped: split into ~chunkSize pieces by
// line, import each, and de-dupe by question text. onProgress(done, total, soFar).
export async function caImportChunked(text, onProgress, chunkSize = 11000) {
  const lines = String(text || "").split(/\n/);
  const chunks = [];
  let cur = "";
  for (const ln of lines) {
    if (cur.length + ln.length + 1 > chunkSize && cur) { chunks.push(cur); cur = ""; }
    cur += ln + "\n";
  }
  ...
  let lastErr = null;
  for (let i = 0; i < chunks.length; i++) {
    onProgress && onProgress(i, chunks.length, out.length);
    let res;
    try { res = await caImport(chunks[i]); } catch (e) { lastErr = e; res = { questions: [] }; }
    for (const q of res.questions || []) {
      const k = String(q.question || "").toLowerCase().replace(/\s+/g, " ").trim().slice(0, 80);
      if (k && !seen.has(k)) { seen.add(k); out.push(q); }
    }
  }
  onProgress && onProgress(chunks.length, chunks.length, out.length);
  // If we got nothing but at least one chunk actually errored (API key/quota/
  // network), surface that instead of a misleading "no questions found".
  if (!out.length && lastErr) throw lastErr;
  return { questions: out };
}

// Study NOTES text -> NEW MCQs testing the facts (not extracting existing Qs).
// `exclude` = already-asked question stems, so batches don't repeat.
export function generateNotesQuiz(text, count = 10, exclude = [], temperature) {
  return post("/api/notes-quiz", { text, count, exclude, temperature }); // -> { title, questions }
}
...
export async function generateQuizChunked(text, onProgress, chunkSize = 11000) {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    } catch (e) { lastErr = e; console.warn("chunk failed", e); }
  }
  // ZERO questions + at least one API failure = the failure IS the story
  // (key/quota/network) — surface it instead of a misleading "kuch nahi mila".
...
// generateStudyPlan lived here — it was the AI Roadmap's only call, and both the
// page and its /api/study-plan route are gone.
...
export async function generateMcqChunked(text, onProgress, chunkSize = 2200) {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    } catch (e) { lastErr = e; console.warn("gk chunk failed", e); }
  }
  // ZERO questions + an API failure = surface the real error, not "kuch nahi mila".
  if (!all.length && lastErr) throw lastErr;
  return { title, questions: all };
}
...
//
// The page is read exactly as before — that already works — and then any
// COLOURED band (a highlighted "correct" option, which a normal pass drops
// because it is white-on-green) is cropped out and read on its own, where
// tesseract gets the polarity right. See lib/ocrprep for what else was tried.
export async function ocrImage(file, onProgress) {
  const Tesseract = (await import("tesseract.js")).default;
  const run = (img, from, to) =>
    Tesseract.recognize(img, "eng", {
      logger: (m) => {
        if (onProgress && m.status === "recognizing text") {
          onProgress(from + (to - from) * m.progress);
        }
      },
    }).then((r) => r.data);

  const page = await run(file, 0, 1);
  const pageText = (page.text || "").trim();

  try {
    const { toCanvas, colourBands, cropBand, linesOf, mergeLines } = await import("./ocrprep");
    const canvas = await toCanvas(file);
    const bands = colourBands(canvas);
    if (!bands.length) return pageText;

    // Collect every band's lines first, keeping real page coordinates, so the
    // merge sorts them back into reading order in one go.
    let extra = [];
    for (let i = 0; i < bands.length; i++) {
      const from = 0.5 + (0.5 * i) / bands.length;
      const to = 0.5 + (0.5 * (i + 1)) / bands.length;
      const data = await run(cropBand(canvas, bands[i]), from, to);
      // cropBand upscales 2x, so halve the y to get back to page coordinates.
      extra = extra.concat(linesOf(data, { yOffset: bands[i].y0, yScale: 0.5 }));
    }
    const merged = mergeLines(linesOf(page), extra).join("\n").trim();
    return merged || pageText;
  } catch (e) {
    console.warn("OCR band pass failed, using the plain read:", e);
    return pageText;
  }
}
...
// ── chapter tagging ─────────────────────────────────────────────────────────
//
// "Ye question kis chapter ka hai" — Answers board ki report ke liye. Text wale
// question ek saath jaate hain (sasta), screenshot wale ek-ek (vision chahiye).
// Dono ek hi route par: /api/chapter-tag.

async function tagPost(payload) {
  const s = getSettings();
  const res = await fetch("/api/chapter-tag", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      ...payload,
      ...
      baseUrl: s.baseUrl,
      ...(geminiActive(s)
        ? { geminiApiKey: s.geminiApiKey, geminiModel: s.geminiModel || "gemini-3-pro" }
        : {}),
    }),
  });
  const data = await res.json();
...
// texts: [{ id, text }] -> [{ id, chapter }]
export function tagChaptersByText({ chapters, texts }) {
  return tagPost({ chapters, texts });
}

// blob: question ki tasveer -> [{ id, chapter }] (ek hi item)
export async function tagChapterByImage({ chapters, id, blob }) {
  const img = await fileToBase64(blob);
  return tagPost({ chapters, image: { id, images: [img] } });
}

// Screenshot ka chapter Gemini ke bina nahi nikalta — panel isse pehle hi
// bata deta hai ki kitne question chhoot jayenge.
export function visionReady() {
  return geminiActive(getSettings());
}
