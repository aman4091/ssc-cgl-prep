...
// Same, for images that already live on R2. The server fetches them (r2.dev
// sends no CORS header, so the browser can't) and hands them to Gemini vision.
// Gemini-only: there is no blob here to feed tesseract.
export async function readImageTextFromUrls(urls) {
  const s = getSettings();
  if (!geminiActive(s)) {
    throw new Error("Image se text padhne ke liye Settings mein Gemini ON karo aur API key daalo.");
  }
  const res = await fetch("/api/vision-ocr", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      ...
      geminiModel: s.geminiModel || "gemini-3-pro",
    }),
  });
