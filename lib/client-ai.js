...
// AI Roadmap coach: ask Gemini for a study plan (macro skeleton + concrete days).
// payload: { mode, examDate, daysLeft, totalPlanDays, fromDayIndex, numDays,
//            profile, catalog, history, metrics }. Returns { macro, days, coachNote }.
export async function generateStudyPlan(payload) {
  const s = getSettings();
  if (!geminiActive(s))
    throw new Error(
      s.geminiApiKey && s.geminiApiKey.trim()
        ? "Gemini abhi OFF hai — Settings mein wapas ON karo (AI Roadmap ke liye zaroori)."
        : "AI Roadmap ke liye Settings mein Gemini API key add karo."
    );
  const res = await fetch("/api/study-plan", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ ...payload, geminiApiKey: s.geminiApiKey, geminiModel: s.geminiModel || "gemini-3-pro" }),
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data.error || "Plan request failed");
  return data; // { macro, days, coachNote }
}
