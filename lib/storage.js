...
export const DEFAULT_SETTINGS = {
  ...
  // The "✨ Gemini" button (poori site mein, ~10 jagah) — kis AI site par khulta
  // hai. Key lib/aisites.js ki AI_SITES list se (gemini/chatgpt/claude/…).
  askAiSite: "gemini",
  // Kisi site ka apna link — jaise Gemini ka doosra account. { gemini: "…" }.
  // Khaali/na ho to lib/aisites ka aam link chalta hai.
  aiSiteUrls: {},
  ...
  // Khaali JAAN-BOOJH kar. Pehle yahan "Is MCQ ka correct answer batao…" pada
  // tha — wo har jagah chala jata tha, notes ke page par bhi, jahan koi MCQ
  // hota hi nahi. Prompt ab sirf Settings se aata hai; khaali chhoda to sirf
  // question/page ka text copy hota hai.
  geminiPrompt: "",
...
export function getSettings() {
  ...
  try {
    const raw = storeGet(KEYS.settings);
...
export function saveSettings(settings) {
  storeSet(KEYS.settings, JSON.stringify(settings));
}
...
export function getQuizzes() {
  ...
  try {
    const raw = storeGet(KEYS.quizzes);
...
function persistQuizzes(all) {
  ...
  while (true) {
    try {
      storeSet(KEYS.quizzes, JSON.stringify(list));
      return list;
    } catch (e) {
...
// Same id dobara save karo to REPLACE hota hai, duplicate nahi banta — jaise
// streaming quiz (lib/similar.js) apna `.questions` badhate hi baar-baar
// saveQuiz bulata hai, ya QBoard beech-mein-chhode quiz ka progress bachata
// hai. Update ki hui quiz sabse aage aati hai (newest-first order).
export function saveQuiz(quiz) {
  const all = getQuizzes().filter((q) => q.id !== quiz.id);
  all.unshift(quiz);
...
export function shedOldQuizzes() {
  ...
  try { storeSet(KEYS.quizzes, JSON.stringify(kept)); }
  catch { persistQuizzes(kept); }
  return true;
}

// Keys that are purely derived / cached / regenerable. Clearing them never loses
// user-created notes, vocab, wrong-book, settings, or sync credentials.
...
// Also wipe any per-type calc "seen" arrays (e.g. cgl.calc.seen.sqroots).
function calcSeenKeys() {
  if (typeof window === "undefined") return [];
  const out = [];
  for (let i = 0; i < localStorage.length; i++) {
    const k = localStorage.key(i);
    if (k && k.startsWith("cgl.calc.seen.")) out.push(k);
  }
  return out;
}

// Remove regenerable/cached localStorage keys to make room for critical data.
// Returns approximate bytes freed (UTF-16 length * 2).
export function freeRegenerableSpace() {
  if (typeof window === "undefined") return 0;
  let freed = 0;
  for (const k of REGENERABLE_KEYS) {
    const v = storeGet(k);
    if (v !== null) {
      freed += v.length * 2;
      storeRemove(k);
...
// Approximate localStorage usage per key. Useful for "storage full" diagnostics.
export function storageUsage() {
  if (typeof window === "undefined") return [];
  const out = [];
  for (let i = 0; i < localStorage.length; i++) {
    const k = localStorage.key(i);
    if (!k || !k.startsWith("cgl.")) continue;
    const v = storeGet(k) || "";
    out.push({ key: k, bytes: v.length * 2 }); // UTF-16 ~2 bytes per char
  }
...
export function deleteQuiz(id) {
  ...
  try { storeSet(KEYS.quizzes, JSON.stringify(all)); } catch { /* full — a delete shrinks it, so this rarely throws */ }
  return all;
}
