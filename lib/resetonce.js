...
// ---------------------------------------------------------------------------
// 🧩 GS prompt v7 — sirf CLUSTER (2026-09-25)
//
// Naya prompt sirf cluster maangta hai (answer pehle se diya hua maan kar),
// aur output ka dhaancha badal gaya: "## 🧩 <naam>" + prose + "**⚡** line".
// Settings mein jo purana default pada hai use ek baar naye se badlo — warna
// ✨ button wahi purana prompt bhejta rahega aur naye cluster bante hi nahi.
//
// Pehchan: naye prompt mein "OUTPUT KA EXACT DHAANCHA" likha hai. Jiske paas
// pehle se naya hai, uska kuch nahi chhua jata.
const GS_V7_FLAG = "cgl.reset.gsprompt7";
const GS_V7_MARK = "OUTPUT KA EXACT DHAANCHA";

export function applyGsPromptV7(gsPrompt) {
  if (typeof window === "undefined" || !gsPrompt) return false;
  try {
    if (localStorage.getItem(GS_V7_FLAG)) return false;
    localStorage.setItem(GS_V7_FLAG, new Date().toISOString());
    const raw = storeGet("cgl.settings");
    const st = raw ? JSON.parse(raw) : {};
    const cur = String((st.shortcutPrompts || {}).gs || "");
    if (cur.includes(GS_V7_MARK)) return false;
    st.shortcutPrompts = { ...(st.shortcutPrompts || {}), gs: gsPrompt };
    storeSet("cgl.settings", JSON.stringify(st));
    try { window.dispatchEvent(new CustomEvent("cgl:settings-changed")); } catch { /* ignore */ }
    return true;
  } catch {
    return false;
  }
