...
// GS prompt v2 (2026-09-13, user ka apna chhota prompt: ANSWER / CLUSTER /
// SSC EXTRA / ONE-LINER). Settings mein koi bhi PURANA default (v0 ya v1
// cluster wala) pada ho to naye se badlo — haath se likha hua ho to nahi.
const GSV2_FLAG = "cgl.reset.gsprompt2";
const GS_V1_MARK = "## 🧩 CLUSTER (fact log mein copy karne ke liye)";

export function applyGsPromptV2(gsPrompt) {
  if (typeof window === "undefined" || !gsPrompt) return false;
  try {
    if (localStorage.getItem(GSV2_FLAG)) return false;
    const raw = storeGet("cgl.settings");
    const st = raw ? JSON.parse(raw) : {};
    const cur = String((st.shortcutPrompts || {}).gs || "");
    if (!cur || cur.startsWith(GS_DEFAULT_START) || cur.includes(GS_V1_MARK)) {
      st.shortcutPrompts = { ...(st.shortcutPrompts || {}), gs: gsPrompt };
      storeSet("cgl.settings", JSON.stringify(st));
    }
    localStorage.setItem(GSV2_FLAG, new Date().toISOString());
    return true;
  } catch {
    return false;
  }
