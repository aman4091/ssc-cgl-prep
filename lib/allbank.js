...
// ── ek hi bank / ek hi chapter ──────────────────────────────────────────
//
// ⚡ Sprint ke liye: "sirf GKTricks ka Statics karna hai" wali baat. Tagging
// wahi hai jo loadAllSubject lagata hai (_src/_chapter/_card), isliye card
// aur meta dono jagah ek jaise dikhte hain.

export function srcOf(subjectSlug, srcId) {
  return sourcesFor(subjectSlug).find((s) => s.id === srcId) || null;
}

export async function partsOf(subjectSlug, srcId) {
  const src = srcOf(subjectSlug, srcId);
  if (!src) return [];
  try { return await src.parts(); } catch { return []; }
}

// partSlug khali ho to us bank ke SAARE chapter.
export async function loadOnePart(subjectSlug, srcId, partSlug) {
  const src = srcOf(subjectSlug, srcId);
  if (!src) return [];
  let parts;
  try { parts = await src.parts(); } catch { return []; }
  const chosen = partSlug ? parts.filter((p) => p.slug === partSlug) : parts;
  const out = [];
  for (const p of chosen) {
    let qs;
    try { qs = await p.load(); } catch { qs = []; }
    (Array.isArray(qs) ? qs : []).forEach((q, j) => {
      out.push({
        ...q,
        _src: src.id, _srcLabel: src.label, _chapter: p.name, _card: src.card,
        _uid: `${src.id}:${p.slug}:${j}`,
      });
    });
  }
  return out;
