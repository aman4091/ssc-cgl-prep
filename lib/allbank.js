...
const gkParts = (subject) => async () => {
  ...
  return (topics || [])
    ...
    .map((t) => ({ name: t.label, load: () => loadGkTopic(t.slug) }));
};
  ...
  maths: [
    {
      ...
      parts: async () => (await loadMathIndex()).chapters.map((c) => ({ name: c.label, load: () => loadMathChapter(c.slug) })),
      count: async () => sum((await loadMathIndex()).chapters),
    },
    {
      id: "maths2025", label: "Maths 2025", icon: "🧮", card: "pyq", href: "/pyq/maths2025",
      parts: async () => ((await loadSscMathsIndex()).chapters || []).map((c) => ({ name: c.label, load: () => loadSscMathsChapter(c.slug) })),
  ...
  reasoning: [
    {
      ...
      parts: async () => (await loadReasonIndex()).chapters.map((c) => ({ name: c.label, load: () => loadReasonChapter(c.slug) })),
  ...
  english: [
    {
      ...
      parts: async () => (await loadEngIndex()).chapters.map((c) => ({ name: c.label, load: () => loadEngChapter(c.slug) })),
      count: async () => sum((await loadEngIndex()).chapters),
    },
    {
      id: "errorpro", label: "Error Pro", icon: "🎯", card: "pyq", href: "/pyq/errorpro",
      parts: async () => (await loadErrorProIndex()).chapters.map((c) => ({ name: c.label, load: () => loadErrorProChapter(c.slug) })),
  ...
  gs: [
    {
      ...
      parts: async () => (await loadWarIndex()).subjects.map((s) => ({ name: s.label, load: () => loadWarSubject(s.slug) })),
