...
/** DeepSeek se saaf ki hui shakl fact ke saath rakh do — ek hi baar banti hai. */
export function setFactFmt(id, fmt) {
  const t = String(fmt || "").trim();
  if (!t) return null;
  const list = read().map((f) => (f.id === id ? { ...f, fmt: t.slice(0, 2000) } : f));
  write(list);
  return list.find((f) => f.id === id) || null;
}

/** Saaf ki hui shakl hata do — wapas asli prose. */
export function clearFactFmt(id) {
  write(read().map((f) => {
    if (f.id !== id) return f;
    const { fmt, ...rest } = f;
    return rest;
...
export function factView(f) {
  // fmt = DeepSeek se sirf shakl sudhari hui copy (facts wahi ke wahi).
  // Ban gayi ho to padhne ke liye wahi, kyunki chipka hua paragraph
  // revision mein aankh se nikal hi nahi pata.
  const prose = String((f && (f.fmt || f.text)) || "").trim();
