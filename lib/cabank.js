// Built-in monthly Current Affairs (public/cabank) — 627 questions with the
// source's own explanations, Jan–Aug 2026, from crazygktrick.com compilations.
//
...
//
// Ye read-only entries hain, feed entry ki shakl mein, taaki Current Affairs
// ka page inhe apne hi drill mein chala sake. Daily bank hata diya gaya —
// ab sirf mahine.
...
// A feed-entry-shaped object for the page. `builtin` marks it read-only.
// Period hamesha mahina hai ("2026-06") — wahi filename /cabank ke andar.
export async function loadCaBankEntry(id) {
  ...
  const m = (index.months || []).find((x) => x.period === period);
  return {
    id,
    feed: "current",
    bucket: "monthly",
    builtin: true,
    date: m?.label || period,   // page ka sar ("July 2026")
