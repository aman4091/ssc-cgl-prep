...
//
// Ek baat jo saaf likhi honi chahiye: delete yahan "record gayab hai" se banta
// hai, "user ne delete dabaya" se nahi. Aakhri sync par record tha aur ab nahi
// hai = delete. Ye tabhi galat ho sakta hai jab data to gayab ho jaye par `sent`
// bach jaye — isliye dono ek hi jagah (IndexedDB) rakhe gaye hain, taaki saath
// jayein. Iska poora ilaaj tab hoga jab har delete apne waqt par tombstone
// likhe (har delete call site par), lekin abhi wo nahi kiya gaya.
//
...
export async function syncOnce({ onProgress } = {}) {
  ...
  const touched = new Set([...Object.keys(stores), ...Object.keys(sent), ...byStore.keys()]);
  const toSend = [];
  const writes = [];
  for (const store of touched) {
    if (LOCAL_ONLY.has(store)) continue;
    const r = reconcileStore(stores[store] ?? null, sent[store], byStore.get(store) || []);
    if (r.changed) writes.push([store, r.nextJson]);
    for (const rec of r.toSend) toSend.push({ store, ...rec });
  ...
  if (rows.length || toSend.length) await addLog(result);
  return result;
}
