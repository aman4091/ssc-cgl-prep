...
export async function pushSync() {
  ...
  const row = { code: getSettings().syncCode, data: { localStorage: collect() }, updated_at: now };
  ...
  saveSettings({ ...getSettings(), syncLastAt: now });
  return now;
}
...
export async function pullSync() {
  ...
  saveSettings({ ...getSettings(), syncLastAt: new Date().toISOString() });
  return rows[0].updated_at || null;
}

// Write pulled keys back, but keep THIS device's own sync credentials so a pull
// never wipes the settings that make syncing work here.
