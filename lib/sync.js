...
function absorbAdditive(remoteLS) {
  ...
  try {
    ...
    const l = JSON.parse(localStorage.getItem("cgl.feed.deleted") || "[]") || [];
    const merged = [...new Set([...l, ...r])];
    if (merged.length !== l.length) { localStorage.setItem("cgl.feed.deleted", JSON.stringify(merged)); changed = true; }
    if (merged.length) {
      const tomb = new Set(merged);
      const ent = JSON.parse(localStorage.getItem("cgl.feed.entries") || "[]") || [];
      const kept = ent.filter((e) => e && !tomb.has(e.id));
      if (kept.length !== ent.length) { localStorage.setItem("cgl.feed.entries", JSON.stringify(kept)); changed = true; }
    }
  } catch { /* ignore */ }
  return changed;
}
...
function mergeWrongInto(remoteLS) {
  ...
  const lBook = JSON.parse(localStorage.getItem("cgl.wrongbook") || "[]") || [];
  ...
  if (bookChanged) localStorage.setItem("cgl.wrongbook", JSON.stringify(merged));
  if (tombChanged) localStorage.setItem(TOMB_KEY, JSON.stringify(tomb));
...
function applyLocalStorage(ls) {
  ...
  try {
    ...
    const l = JSON.parse(localStorage.getItem("cgl.feed.deleted") || "[]") || [];
    const merged = [...new Set([...l, ...r])];
    localStorage.setItem("cgl.feed.deleted", JSON.stringify(merged));
    if (merged.length) {
      const tomb = new Set(merged);
      const ent = JSON.parse(localStorage.getItem("cgl.feed.entries") || "[]") || [];
      const kept = ent.filter((e) => e && !tomb.has(e.id));
      if (kept.length !== ent.length) localStorage.setItem("cgl.feed.entries", JSON.stringify(kept));
    }
  } catch { /* ignore */ }
  try {
    saveSettings({
