...
import { SUBJECTS, SUGGESTED, getChapters } from "./grammar";
...
export function blankRoadmap() {
  return {
    examDate: "", startKey: "", onboarded: false,
    profile: {
      hoursWeekday: 4, hoursWeekend: 6, studyWindowsText: "", offDays: [],
      weakSubjects: [], strongSubjects: [], mocksTotalAvailable: 73,
      startVocabDay: 1, notes: "",
    },
...
export function taskHref(task, bankIndex = [], rm = getRoadmap()) {
  ...
  if (k === "theory") {
    const subj = task.ref?.subject || "math";
    const name = (task.ref?.chapter || "").toLowerCase();
    const ch = name ? getChapters(subj).find((c) => c.name.toLowerCase() === name) : null;
...
export function materializeToday(bankIndex = []) {
  ...
  // 3) add today's pending tasks (in plan order -> first is #1)
  if (day) {
