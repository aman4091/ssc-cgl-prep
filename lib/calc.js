// Local speed-math generator (no AI). Fast calculation drills for SSC CGL.
import { makeId } from "./storage";

import { storeGet, storeSet, storeRemove } from "./bigstore";
function ri(min, max) { return Math.floor(Math.random() * (max - min + 1)) + min; }
function shuffle(a) {
  const arr = [...a];
  for (let i = arr.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [arr[i], arr[j]] = [arr[j], arr[i]]; }
  return arr;
}

// numeric MCQ: one correct + 3 near distractors
function numMcq(question, answer, explanation, type, spread) {
  const sp = spread || Math.max(2, Math.round(Math.abs(answer) * 0.06) + 2);
  const set = new Set([answer]);
  let guard = 0;
  while (set.size < 4 && guard < 80) {
    const d = answer + (Math.random() < 0.5 ? -1 : 1) * ri(1, sp + 3);
    if (d !== answer && d >= 0) set.add(d);
    guard++;
  }
  let extra = 1;
  while (set.size < 4) { set.add(answer + extra); extra++; }
  const opts = shuffle([...set]);
  return { question, options: opts.map(String), answer: opts.indexOf(answer), explanation, type, diagram: "" };
}

// string MCQ (fractions / percent): correct + distractors from a pool
function strMcq(question, answerStr, pool, explanation, type) {
  const set = new Set([answerStr]);
  for (const d of shuffle(pool)) { if (set.size >= 4) break; if (d && d !== answerStr) set.add(d); }
  let i = 0;
  while (set.size < 4) { set.add(answerStr + " "); i++; if (i > 5) break; }
  const opts = shuffle([...set]);
  return { question, options: opts, answer: opts.indexOf(answerStr), explanation, type, diagram: "" };
}

function gcd(a, b) { a = Math.abs(a); b = Math.abs(b); while (b) { [a, b] = [b, a % b]; } return a || 1; }
function fr(num, den) { if (den < 0) { num = -num; den = -den; } const g = gcd(num, den) || 1; return { num: num / g, den: den / g }; }
function frStr(f) { return f.den === 1 ? String(f.num) : `${f.num}/${f.den}`; }
function digitalRoot(n) { n = Math.abs(n); return n === 0 ? 0 : (n % 9 === 0 ? 9 : n % 9); }

const P2F = [
  ["12.5%", "1/8"], ["25%", "1/4"], ["50%", "1/2"], ["75%", "3/4"], ["20%", "1/5"], ["40%", "2/5"],
  ["60%", "3/5"], ["80%", "4/5"], ["33.33%", "1/3"], ["66.67%", "2/3"], ["16.67%", "1/6"], ["10%", "1/10"],
  ["37.5%", "3/8"], ["62.5%", "5/8"], ["87.5%", "7/8"], ["6.25%", "1/16"], ["11.11%", "1/9"], ["14.28%", "1/7"], ["8.33%", "1/12"],
];

const GENERATORS = {
  add2: () => { const a = ri(10, 99), b = ri(10, 99); return numMcq(`${a} + ${b} = ?`, a + b, `${a} + ${b} = ${a + b}`, "add2"); },
  add3: () => { const a = ri(100, 999), b = ri(100, 999); return numMcq(`${a} + ${b} = ?`, a + b, `${a} + ${b} = ${a + b}`, "add3"); },
  sub: () => { const a = ri(100, 999), b = ri(10, a); return numMcq(`${a} − ${b} = ?`, a - b, `${a} − ${b} = ${a - b}`, "sub"); },
  mul2: () => { const a = ri(11, 99), b = ri(11, 99); return numMcq(`${a} × ${b} = ?`, a * b, `${a} × ${b} = ${a * b}`, "mul2"); },
  mul3: () => { const a = ri(101, 999), b = ri(11, 99); return numMcq(`${a} × ${b} = ?`, a * b, `${a} × ${b} = ${a * b}`, "mul3"); },
  // Multiplication tables (pahade) 1–30, each up to ×10. Distractors sit a row
  // either side (spread ~n) so the choices are the neighbouring multiples.
  table: () => { const n = ri(2, 30), m = ri(2, 10); return numMcq(`${n} × ${m} = ?`, n * m, `${n} × ${m} = ${n * m}  (${n} ka table)`, "table", n); },
  square: () => { const a = ri(1, 75); return numMcq(`${a}² = ?`, a * a, `${a}² = ${a * a}`, "square"); },
  cube: () => { const a = ri(1, 30); return numMcq(`${a}³ = ?`, a * a * a, `${a}³ = ${a * a * a}`, "cube"); },
  // The hard cubes — 21³ (9261) … 30³ (27000) — worth drilling on their own.
  cubeHard: () => { const a = ri(21, 30); return numMcq(`${a}³ = ?`, a * a * a, `${a}³ = ${a * a * a}`, "cubeHard"); },
  sqrt: () => { const a = ri(1, 40); const n = a * a; return numMcq(`√${n} = ?`, a, `√${n} = ${a}  (kyunki ${a}² = ${n})`, "sqrt", 3); },
  cbrt: () => { const a = ri(1, 30); const n = a * a * a; return numMcq(`∛${n} = ?`, a, `∛${n} = ${a}  (kyunki ${a}³ = ${n})`, "cbrt", 3); },
  divisibility: () => {
    const d = [3, 4, 6, 7, 8, 9, 11][ri(0, 6)];
    const n = ri(100, 9999);
    const yes = n % d === 0;
    const opts = ["Haan (Yes)", "Nahi (No)"];
    return { question: `Kya ${n}, ${d} se divisible hai?`, options: opts, answer: yes ? 0 : 1, explanation: `${n} ÷ ${d} = ${(n / d).toFixed(2)} → ${yes ? "divisible" : "nahi"}`, type: "divisibility", diagram: "" };
  },
  percent2fraction: () => {
    const [p, f] = P2F[ri(0, P2F.length - 1)];
    const pool = P2F.filter(([, ff]) => ff !== f).map(([, ff]) => ff);
    return strMcq(`${p} = kaunsa fraction?`, f, pool, `${p} = ${f}`, "percent2fraction");
  },
  approx: () => {
    const a = ri(2, 9) * 100 + ri(-6, 6);
    const b = ri(11, 89);
    const actual = a * b;
    const ans = Math.round(actual / 100) * 100;
    // distractors are ALSO round hundreds so the answer isn't an obvious giveaway
    const set = new Set([ans]);
    let step = 1;
    while (set.size < 4) { set.add(ans + step * 100); if (ans - step * 100 >= 0) set.add(ans - step * 100); step++; }
    const opts = shuffle([...set].slice(0, 4).includes(ans) ? [...set].slice(0, 4) : [ans, ...[...set].filter((x) => x !== ans).slice(0, 3)]);
    return { question: `${a} × ${b} ≈ ? (nearest 100)`, options: opts.map(String), answer: opts.indexOf(ans), explanation: `${a} × ${b} = ${actual} ≈ ${ans}`, type: "approx", diagram: "" };
  },
  digitsum: () => {
    const n = ri(1000, 999999);
    const dr = digitalRoot(n);
    const opts = shuffle([dr, ((dr) % 9) + 1, ((dr + 1) % 9) + 1, ((dr + 3) % 9) + 1].filter((v, i, arr) => arr.indexOf(v) === i));
    while (opts.length < 4) { const c = ri(1, 9); if (!opts.includes(c)) opts.push(c); }
    const o = shuffle(opts.slice(0, 4).includes(dr) ? opts.slice(0, 4) : [dr, ...opts.slice(0, 3)]);
    return { question: `${n} ka digit-sum (single digit / digital root) = ?`, options: o.map(String), answer: o.indexOf(dr), explanation: `Digits jodo baar-baar → ${dr}`, type: "digitsum", diagram: "" };
  },
  fractions: () => {
    let b = ri(2, 12), d = ri(2, 12);
    let a = ri(1, b - 1), c = ri(1, d - 1);
    const ops = ["+", "−", "×", "÷"];
    const op = ops[ri(0, 3)];
    if (op === "−" && a * d < c * b) { [a, b, c, d] = [c, d, a, b]; } // keep positive
    let num, den;
    if (op === "+") { num = a * d + c * b; den = b * d; }
    else if (op === "−") { num = a * d - c * b; den = b * d; }
    else if (op === "×") { num = a * c; den = b * d; }
    else { num = a * d; den = b * c; }
    const res = fr(num, den);
    const ans = frStr(res);
    const pool = [
      frStr(fr(res.num + 1, res.den)),
      frStr(fr(Math.max(0, res.num - 1), res.den)),
      frStr(fr(res.num, res.den + 1)),
      frStr(fr(res.num + res.den, res.den)),
      frStr(fr(res.den, res.num || 1)),
    ];
    return strMcq(`${a}/${b} ${op} ${c}/${d} = ?`, ans, pool, `= ${ans}`, "fractions");
  },
};

export const CALC_TYPES = [
  { key: "add2", label: "2-digit +", icon: "➕" },
  { key: "add3", label: "3-digit +", icon: "➕" },
  { key: "sub", label: "Subtraction", icon: "➖" },
  { key: "mul2", label: "2-digit ×", icon: "✖️" },
  { key: "mul3", label: "3-digit ×", icon: "✖️" },
  { key: "table", label: "Tables (1–30)", icon: "🔢" },
  { key: "square", label: "Squares (1–75)", icon: "²" },
  { key: "cube", label: "Cubes (1–30)", icon: "³" },
  { key: "cubeHard", label: "Cubes (21–30)", icon: "³" },
  { key: "sqrt", label: "Square roots", icon: "√" },
  { key: "cbrt", label: "Cube roots", icon: "∛" },
  { key: "divisibility", label: "Divisibility", icon: "➗" },
  { key: "percent2fraction", label: "% → Fraction", icon: "％" },
  { key: "approx", label: "Approximation", icon: "≈" },
  { key: "digitsum", label: "Digit sum", icon: "🔢" },
  { key: "fractions", label: "Fractions ± × ÷", icon: "½" },
];
const ALL_KEYS = CALC_TYPES.map((t) => t.key);

export function calcTypeLabel(key) { return (CALC_TYPES.find((t) => t.key === key) || {}).label || key; }

// ---- no-repeat generated questions ----------------------------------------
// The generators pick random operands, so small-domain types (tables 2–30, cubes
// 1–30, %→fraction…) throw up the SAME question ("3 × 7") across drills. We keep a
// per-type window of recently-served questions in localStorage and skip anything in
// it. The window (400) is larger than every small domain, so those cycle fully
// before any repeat; when a type genuinely runs out of fresh ones, we reset it and
// a new cycle starts.
const SEEN_KEY = "cgl.calc.seen.";
const SEEN_CAP = 400;
function readSeenArr(type) { try { const r = storeGet(SEEN_KEY + type); return r ? JSON.parse(r) : []; } catch { return []; } }
function writeSeenArr(type, arr) { try { storeSet(SEEN_KEY + type, JSON.stringify(arr)); } catch { /* quota / SSR */ } }

// Generate one question of type k whose text hasn't been served recently (or in
// this drill). `sessionSet` de-dups within the current drill.
function genFresh(k, sessionSet) {
  const gen = GENERATORS[k] || GENERATORS.add2;
  let arr = readSeenArr(k);
  let seen = new Set(arr);
  let q, sig, tries = 0;
  do { q = gen(); sig = q.question; tries++; }
  while ((seen.has(sig) || sessionSet.has(sig)) && tries < 40);
  if (tries >= 40) { arr = []; }              // domain exhausted → fresh cycle
  arr.push(sig);
  if (arr.length > SEEN_CAP) arr = arr.slice(arr.length - SEEN_CAP);
  writeSeenArr(k, arr);
  sessionSet.add(sig);
  return q;
}

// one question from the given types (default: all)
export function randomCalcQuestion(keys) {
  const pool = (keys && keys.length ? keys : ALL_KEYS).filter((k) => GENERATORS[k]);
  const k = pool[ri(0, pool.length - 1)];
  return genFresh(k, new Set());
}

export function buildCalcQuestions(keys, count = 20) {
  const pool = (keys && keys.length ? keys : ALL_KEYS).filter((k) => GENERATORS[k]);
  const n = Math.min(Math.max(count, 1), 100);
  const session = new Set();
  const out = [];
  for (let i = 0; i < n; i++) {
    const k = pool[i % pool.length];
    out.push(genFresh(k, session));
  }
  return shuffle(out);
}

// ---- uploaded question pool (from PDFs/images) mixed randomly into drills ----
const CALC_POOL_KEY = "cgl.calc.pool";
export function getCalcPool() {
  if (typeof window === "undefined") return [];
  try { const r = storeGet(CALC_POOL_KEY); return r ? JSON.parse(r) : []; }
  catch { return []; }
}
export function addCalcPoolQuestions(questions) {
  const clean = (questions || []).filter((q) => q && q.question && Array.isArray(q.options) && q.options.length >= 2);
  const all = getCalcPool().concat(clean);
  storeSet(CALC_POOL_KEY, JSON.stringify(all));
  return clean.length;
}
export function clearCalcPool() { storeRemove(CALC_POOL_KEY); }

// ---- book bank (public/calcbank) ----
// 3,355 real questions from "Calculation ka Best Compilation". They live as static
// files, not in localStorage: 700KB would blow past the storage quota and would be
// pushed through cloud sync on every write, for data that never changes per user.
export const BOOK_PREFIX = "book-";
export function isBookKey(k) { return typeof k === "string" && k.startsWith(BOOK_PREFIX); }

let bankIndexCache = null;
const topicCache = {};

// -> [{ key, label, icon, count }]
export async function loadCalcBankIndex() {
  if (bankIndexCache) return bankIndexCache;
  try {
    const res = await fetch("/calcbank/index.json");
    if (!res.ok) throw new Error(String(res.status));
    const data = await res.json();
    bankIndexCache = data.topics || [];
  } catch { bankIndexCache = []; }
  return bankIndexCache;
}

async function loadTopic(key) {
  if (topicCache[key]) return topicCache[key];
  try {
    const res = await fetch(`/calcbank/${encodeURIComponent(key)}.json`);
    if (!res.ok) throw new Error(String(res.status));
    topicCache[key] = await res.json();
  } catch { topicCache[key] = []; }
  return topicCache[key];
}

// Random questions drawn evenly across the given book topics.
export async function loadBookQuestions(keys, count) {
  const topics = (keys || []).filter(isBookKey);
  if (!topics.length || count <= 0) return [];
  const lists = await Promise.all(topics.map(loadTopic));
  const out = [];
  const pools = lists.map((l) => shuffle(l));
  // round-robin so one huge topic doesn't crowd out the others
  while (out.length < count) {
    let added = false;
    for (const p of pools) {
      if (out.length >= count) break;
      if (p.length) { out.push(p.pop()); added = true; }
    }
    if (!added) break;   // every topic exhausted
  }
  return out;
}

// `keys` may mix generator keys (add2, mul3…) and book topic keys (book-decimal…).
// Async because the book topics are fetched on demand.
export async function buildCalcQuiz(keys, count = 20, secPerQ = 12) {
  const all = keys && keys.length ? keys : ALL_KEYS;
  const genKeys = all.filter((k) => !isBookKey(k));
  const bookKeys = all.filter(isBookKey);

  // Split the requested count between generated and book questions by how many of
  // each kind were selected, so picking only book topics gives an all-book drill.
  const bookShare = bookKeys.length && genKeys.length
    ? Math.round((count * bookKeys.length) / (bookKeys.length + genKeys.length))
    : bookKeys.length ? count : 0;

  const book = await loadBookQuestions(bookKeys, bookShare);
  const genCount = count - book.length;
  const local = genKeys.length && genCount > 0 ? buildCalcQuestions(genKeys, genCount) : [];

  let questions = shuffle([...local, ...book]);

  // blend in some of the user's own uploaded questions (up to ~40%)
  const pool = getCalcPool();
  if (pool.length) {
    const take = Math.min(pool.length, Math.max(1, Math.ceil(count * 0.4)));
    questions = shuffle([...questions, ...shuffle(pool).slice(0, take)]).slice(0, count);
  }

  return {
    id: makeId(),
    title: "Calculation Speed Drill",
    source: "calc",
    createdAt: new Date().toISOString(),
    questions,
    timeLimitSec: questions.length * secPerQ, // speed: default 12s per question
  };
}
