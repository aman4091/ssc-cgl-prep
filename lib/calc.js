...
const SEEN_CAP = 400;
function readSeenArr(type) { try { const r = localStorage.getItem(SEEN_KEY + type); return r ? JSON.parse(r) : []; } catch { return []; } }
function writeSeenArr(type, arr) { try { localStorage.setItem(SEEN_KEY + type, JSON.stringify(arr)); } catch { /* quota / SSR */ } }
