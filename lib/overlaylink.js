...
export async function overlayAsk({ text = "", imageB64 = "", imageUrl = "", subject = "", jobId = "" }) {
  ...
  try {
    const r = await fetch(`${b}/run/ask`, {
      ...
      body: JSON.stringify({ text, image_b64: imageB64, image_url: imageUrl, subject, job_id: jobId }),
    });
    return r.ok;
  } catch { return false; }
}

/** -> { ready, text, job } — jawab aaya ya nahi (aaya to ek hi baar).
    `job` wahi number hai jo overlayAsk ke saath bheja tha. */
export async function overlayAnswer() {
  ...
  try {
    ...
    if (!r.ok) return { ready: false, text: "", job: "" };
    return await r.json();
  } catch { return { ready: false, text: "", job: "" }; }
}
...
// 🔒 Focus lock — PC par sirf apni site/overlay chalti rahe (over/focus.py).
// Overlay band ho to sab chupchaap `null` — site waise ki waisi chalti hai.
export async function focusStart(min) {
  const b = await overlayBase();
  if (!b) return null;
  try {
    const r = await fetch(`${b}/focus/start`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function focusStop() {
  ...
  if (!b) return null;
  try {
    const r = await fetch(`${b}/focus/stop`, { method: "POST" });
    return r.ok ? r.json() : null;
  } catch { return null; }
}

export async function focusLog() {
  const b = await overlayBase();
  if (!b) return null;
  try {
    const r = await fetch(`${b}/focus/log`, { cache: "no-store" });
    return r.ok ? r.json() : null;
  } catch { return null; }
}
...
export async function focusState() {
  ...
  if (!b) return null;
  try {
    const r = await fetch(`${b}/focus/state`, { cache: "no-store" });
    return r.ok ? r.json() : null;
  } catch { return null; }
}
