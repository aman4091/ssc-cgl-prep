// Har word ka chhota matlab — DeepSeek se, background mein.
//
...
// matlab banwa lete hain, aur patti par wahi jata hai. Baaki word (OWS,
// idiom, vocab) bhi isi se guzarte hain — unki apni angrezi `def` tab tak
// chalti hai jab tak unka Hinglish wala matlab nahi ban jata.
//
// Kaam din ke kram mein hota hai aur roz paanch din tak seemit hai (lib/vocab
// ka allowedUptoDay) — aaj Day 1-5, kal Day 6-10. Chaar hazaar word ek
// baithak mein banwane ka na matlab hai na zaroorat.
//
...
import { wordsMissingLine, addVocabLines } from "./vocab";
...
export async function fillVocabLines({ force = false, limit = BATCH } = {}) {
  ...
  const items = wordsMissingLine(limit);
  ...
  try {
    const res = await fetch("/api/vocab-lines", {
      ...
      body: JSON.stringify({
        items, apiKey: s.apiKey, model: s.model, baseUrl: s.baseUrl,
      }),
    });
    if (!res.ok) return 0;
...
export { pendingLineCount } from "./vocab";
