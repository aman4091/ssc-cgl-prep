...
export const SUGGESTED = {
  ...
  // "Ancient History" and "Polity" match ready-made banks (see lib/gkbank) —
  // creating a chapter by that name is what makes its GK Tricks tab appear.
  gs: ["History", "Ancient History", "Geography", "Polity", "Economy", "General Science", "Static GK", "Current Affairs"],
...
// Same key, exported so callers can remove a specific question stably (not by index).
export function chapterQuestionKey(q) { return qKey(q); }
...
export function removeChapterQuestionByKey(chapterId, key) {
  const all = read(GKEYS.questions, {});
  all[chapterId] = (all[chapterId] || []).filter((q) => qKey(q) !== key);
  write(GKEYS.questions, all);
}
// Replace a question (matched by its old key) with an edited version.
export function updateChapterQuestion(chapterId, key, newQ) {
  const all = read(GKEYS.questions, {});
  all[chapterId] = (all[chapterId] || []).map((q) => (qKey(q) === key ? { ...q, ...newQ } : q));
  write(GKEYS.questions, all);
}
