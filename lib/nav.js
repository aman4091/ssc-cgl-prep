...
export const NAV_GROUPS = [
  ...
  },
  {
    ...
    name: "GKTricks",
    icon: "🧠",
    // gkbank holds both shelves; GS is this one, english is Mirror below.
...
export function groupForPath(pathname) {
  ...
  for (const g of NAV_GROUPS) {
    // `match` may be one prefix or several — gkbank's two shelves share a route
    // and are told apart by slug.
    for (const m of [].concat(g.match || [])) {
