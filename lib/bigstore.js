...
export const BIG_KEYS = [
  ...
  "cgl.quicktrick",
  "cgl.answersdrill",
  // ⚡ Sprint: ho-gaye question (sirf hash), ★ bookmark, aur tasveer wale
  // question ke DeepSeek jawab. Teeno badhte rehte hain.
  "cgl.sprint.done",
  "cgl.sprint.marks",
  "cgl.sprint.ans",
  "cgl.sprint.sets",
  // 📝 Notes ke page par ✨ dabane ke baad paste kiye hue one-liner.
  "cgl.pastednotes",
  // 💬 Poochho panel ki purani baatcheet (jawab + tasveeron ke pate).
  "cgl.ask.threads",
];
...
// ---- Ek hi browser ki KAI tabs ----
//
// Har tab boot par IDB ek baar padh kar `cache` mein rakhti hai, aur uske baad
// sirf apna cache padhti hai. Do tab khuli hon to ek ka likha doosri ko kabhi
// dikhta hi nahi tha — aur jab doosri apna kuch likhti, wo apni PURANI poori
// list IDB par wapas likh deti. Pehli tab ka kaam (jaise overlay ka qid
// chipkana) chupchaap mit jata. Sync bhi ise nahi bachata: dono tab ek hi
// device id se chalti hain, aur apne device ki row wapas padhi hi nahi jaati.
//
// Ab har likha hua big key baaki tabs ko turant bheja jata hai, aur wo apna
// cache wahi kar leti hain. Unka apna adhoora (debounced) IDB write radd —
// likhne wali tab naya roop khud likhegi, aur purana roop us naye ko mita deta.
let chan = null;
const heard = new Set();       // hydrate ke dauraan doosri tab se aaye keys
function channel() {
  if (chan !== null) return chan;
  chan = false;
  try {
    if (typeof BroadcastChannel === "undefined") return chan;
    chan = new BroadcastChannel("cgl-bigstore");
    chan.onmessage = (e) => {
      const d = e.data || {};
      // Nayi tab khuli hai: jo abhi IDB tak nahi pahuncha (debounce mein
      // atka hai) wo use seedha de do, warna wo purana IDB padh leti.
      if (d.hello) {
        for (const k of timers.keys()) if (cache.has(k)) announce(k, cache.get(k));
        return;
      }
      const { key, value } = d;
      if (!BIG.has(key)) return;
      clearTimeout(timers.get(key));
      timers.delete(key);
      if (!hydrated) heard.add(key);
...
function announce(key, value) {
  const c = channel();
  if (!c) return;
  try { c.postMessage({ key, value }); } catch { /* ignore */ }
}
...
export function storeRemove(key) {
  ...
  if (BIG.has(key)) { cache.delete(key); persist(key); announce(key, null); }
...
// Har baar localStorage se TAAZA padhte hain aur turant likhte hain (chhoti si
// cheez hai). Pehle ye bhi tab ki apni yaad mein rehta tha — to ek tab ka darj
// kiya delete doosri tab apni purani copy likh kar mita deti, aur wo delete
// kabhi cloud tak nahi jata (record doosre devices par zinda reh jata).
let tombs = null;              // { store: [recordId] }
let capture = true;
...
export async function hydrateStore() {
  ...
  // Pehle sunna shuru — padhne ke dauraan doosri tab kuch likhe to wo bhi mile.
  const c = channel();
  if (c) { try { c.postMessage({ hello: true }); } catch { /* ignore */ } }
  try {
    db = await openDb();
    const rows = await idbAll();
    // Jo key doosri tab ne abhi-abhi bheji, wo IDB wale se nayi hai.
    for (const [k, v] of rows) if (BIG.has(k) && !heard.has(k)) cache.set(k, v);
  } catch {
