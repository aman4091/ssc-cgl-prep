...
import { loadMixCatalog, buildMixTest, minutesFor } from "./mixtest";
...
// ⚡ Auto Practice — ek button, aur SAARE subject ke kamzor chapter mila kar
// ek chhota mixed test. "Aaj ka set" ke upar ek yahi is liye ki wo per-subject
// hai (chuno pehle KAUNSA subject); ye subject hi nahi poochta — jo bhi
// sabse kamzor hai (koi bhi subject ho) wahi pehle aata hai. lib/mixtest ka
// wahi engine istemal hota hai jo "🧪 Apna test banao" chalata hai, isliye
// har question apne asli subject/chapter ke naam se hi Mistake Notebook mein
// jata hai (na ki "Auto Practice" ke naam se).
export async function buildAutoPractice(n = 10) {
  const catalog = await loadMixCatalog();
  const allParts = catalog.flatMap((subj) => subj.sources.flatMap((src) => src.parts));
  if (!allParts.length) return null;

  const norm = (s) => String(s || "").toLowerCase().replace(/[^a-z0-9]+/g, "");
  const weak = getWeakAreas(); // sabse zyada galat pehle
  const used = new Set();
  const picks = [];
  let remaining = n;
  const perChapter = Math.max(2, Math.ceil(n / 4));

  // Pehle wahi chapter jinme sach mein galtiyan ho rahi hain (naam se match).
  for (const w of weak) {
    if (remaining <= 0) break;
    const wn = norm(w.category);
    if (!wn) continue;
    const match = allParts.find((p) => !used.has(p.key) && norm(p.name) === wn);
    if (!match) continue;
    used.add(match.key);
    const take = Math.min(remaining, perChapter);
    picks.push({ key: match.key, n: take });
    remaining -= take;
  }
  // Koi weak-area match na mile (naya user) ya set abhi bhi khaali — baaki
  // random chapters se bhar do, taaki button hamesha kaam kare.
  let guard = 0;
  while (remaining > 0 && allParts.length > used.size && guard++ < 40) {
    const p = allParts[Math.floor(Math.random() * allParts.length)];
    if (used.has(p.key)) continue;
    used.add(p.key);
    const take = Math.min(remaining, perChapter);
    picks.push({ key: p.key, n: take });
    remaining -= take;
  }
  if (!picks.length) return null;

  const made = await buildMixTest(picks, { minutes: minutesFor(n), mix: true });
  if (!made) return null;
  return made; // { id, title, n, rows, subject }
}
