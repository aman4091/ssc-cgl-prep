...
// Free an entry's off-localStorage images (R2 objects / IndexedDB blobs) so a
// delete actually reclaims the space. Best-effort, fire-and-forget.
async function freeImages(entry) {
  for (const im of entry?.images || []) {
    try { if (im?.url) await deleteFromR2(im.url); else if (im?.id) await deleteFile(im.id); }
    catch { /* ignore */ }
  }
}
...
// Add one entry: a subject + topic label, text, and images. Images are stored
// OUTSIDE localStorage (R2 URL, or an IndexedDB blob id) — the entry keeps only
// the tiny `images` descriptors, so a big screenshot no longer blows the
// localStorage/sync quota. `img` (a legacy inline data-URL) is still read for
// old notes but not written for new ones.
export function addEntry({ title = "", subject = "", topic = "", text = "", img = "", images = [] } = {}) {
  const entry = {
    id: makeId(),
    ...
    img: img || "",                                   // legacy inline data-URL
    images: Array.isArray(images) ? images : [],      // {url}|{id} descriptors
...
export function deleteEntry(id) {
  ...
  if (gone) freeImages(gone); // reclaim R2 / IndexedDB space in the background
  return saveEntries(all.filter((e) => e.id !== id));
}
...
export function fileLegacyEntries() {
  if (typeof window === "undefined") return;
  try {
    if (storeGet(FILED_KEY)) return;
    const list = getEntries();
    if (list.length) {
      if (!storeGet(BACKUP_KEY)) {
        try { storeSet(BACKUP_KEY, JSON.stringify(list)); } catch { /* backup is best-effort */ }
      }
    ...
    storeSet(FILED_KEY, "1");
  } catch {
    /* quota / private mode — leave the notes as-is */
  }
}

// The pre-filing snapshot, if this device made one before filing ran.
export function readFilingBackup() {
  if (typeof window === "undefined") return [];
  try {
    const raw = storeGet(BACKUP_KEY);
...
// Heal notes whose title was dropped by an early version of the filing: for every
// current note with an empty title, look up its original by id in the backup and
// restore the heading it used to carry (old title → subject → topic). Only fills
// blanks, matches by id, so it's safe to run on every mount. Returns how many it
// brought back, and whether a backup even existed to work from.
export function restoreTitlesFromBackup() {
  const backup = readFilingBackup();
  if (!backup.length) return { hadBackup: false, restored: 0 };
  const byId = new Map(backup.map((e) => [e.id, e]));
  let restored = 0;
  const next = getEntries().map((e) => {
    if (String(e.title || "").trim()) return e;
    const old = byId.get(e.id);
    const t = old ? String(old.title || old.subject || old.topic || "").trim() : "";
    if (t) { restored += 1; return { ...e, title: t }; }
    return e;
  });
  if (restored) saveEntries(next);
