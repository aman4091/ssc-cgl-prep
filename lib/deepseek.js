...
export async function deepseekChat({
  ...
  // The reasoner (R1) family rejects response_format; every other model takes it.
  // Gate on that, not on the name containing "chat" — otherwise any new model id
  // silently loses JSON mode.
  const useJson = jsonMode && !(model || "deepseek-chat").includes("reasoner");
  ...
  return { ok: true, content, status: 200, finishReason };
}
