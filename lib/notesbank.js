...
const BOOKS = {
  ...
  goldenrules: {
    ...
    sub:
      "RBE ka 50 Golden Rules — 576 rule, 14 chapter (Nouns se Question Tags " +
      "tak), har rule ke saath uske apne example. Chapter ka naam menu mein hai; " +
      "page wahi hai jo chhapi kitaab ka hai.",
    note:
...
export async function loadNotes(book) {
  ...
  try {
    ...
    const out = {
      ...
      gemini: !!cfg.gemini,
      // "हिंदी" button — is book ke page ka apna Hinglish paste karke
      // side-by-side padhna. Pehle ye reader ke andar slug se tay hota tha
      // (`slug.startsWith("parmar-")`), isliye nayi book jodne par button
      // chupchaap gayab rehta tha aur pata bhi nahi chalta tha. Ab book apni
      // config mein khud batati hai.
      hinglish: !!cfg.hinglish,
      eyebrow: cfg.eyebrow,
      title: cfg.title,
      sub: cfg.sub,
      note: cfg.note,
