...
// ✓ Jo question RUN se ban chuka — uski chhoti pehchaan yahan rehti hai.
//
// Owner ka niyam do hisson mein hai: purana jawab (kahin se bhi aaya ho) run
// ko rokta nahi — run wala jawab hi rakhna hai — PAR jo ek baar run se ban
// gaya, wo dobara run mein na aaye. Isliye "jawab hai ya nahi" se kaam nahi
// chalta; yaad rakhna padta hai ki wo jawab RUN ne banaya tha.
//
// `cgl.airun.done` — { dkey: din }. Baaki cgl. keys ki tarah sync hoti hai.
const RUN_DONE = "cgl.airun.done";

function runDoneMap() {
  if (typeof window === "undefined") return {};
  try {
    const v = JSON.parse(storeGet(RUN_DONE) || "{}");
    return v && typeof v === "object" && !Array.isArray(v) ? v : {};
  } catch { return {}; }
}

export function markRunDone(dkey) {
  if (!dkey) return;
  const all = runDoneMap();
  if (all[dkey]) return;
  all[dkey] = new Date().toISOString().slice(0, 10);
  try { storeSet(RUN_DONE, JSON.stringify(all)); } catch { /* quota */ }
}

/** Ek jagah ka nishaan hata do — "ye chapter dobara run se karwana hai". */
export function clearRunDone(dkeys) {
  const all = runDoneMap();
  let n = 0;
  for (const k of dkeys || []) if (all[k]) { delete all[k]; n += 1; }
  if (n) { try { storeSet(RUN_DONE, JSON.stringify(all)); } catch { /* quota */ } }
  return n;
}
...
export const AI_SOURCES = [
  ...
  // 📖 Answers ke screenshot — jinka abhi koi Gemini answer nahi hai.
  {
    ...
    jobs: async (slug) => {
      ...
      // 🔴 Hard wale card board ki aam list se BAHAR hain (unki apni
      // chhaanti hai). Run unhe le leta tha, isliye run ka pehla question
      // board ke pehle se mel hi nahi khata tha.
      const hard = getHardSet();
      const qnum = (r) => {
      ...
      return getWrongBook()
        .filter((r) => r.subject === slug && !hard.has(r.id))
 ...
 *
 * Pehle se jawab hone se koi question chhoota nahi — run ka jawab purane ke
 * upar likh jata hai. Par jo EK BAAR RUN SE ban chuka, wo dobara nahi aata
 * (nishaan `cgl.airun.done` mein), aur ek hi question list mein do baar ho to
 * bhi ek hi baar.
 */
export async function loadJobs(source, slug, limit, item) {
  ...
  const done = runDoneMap();
  const seen = new Set();
  const left = ordered.filter((j) => {
    if (done[j.dkey]) return false;        // run se pehle hi ban chuka
    if (seen.has(j.dkey)) return false;    // wahi question dobara
    seen.add(j.dkey);
    return true;
  });
