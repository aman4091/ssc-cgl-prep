...
export function randomCurrentAffairsQuestion() {
  ...
  let seen;
  try { seen = new Set(JSON.parse(localStorage.getItem(CA_CYCLE_KEY) || "[]").filter((k) => byKey.has(k))); }
  catch { seen = new Set(); }
  let unseen = allKeys.filter((k) => !seen.has(k));
  if (unseen.length === 0) { seen = new Set(); unseen = allKeys; } // cycle complete -> reset
  const pick = unseen[Math.floor(Math.random() * unseen.length)];
  seen.add(pick);
  try { localStorage.setItem(CA_CYCLE_KEY, JSON.stringify([...seen])); } catch { /* ignore */ }
...
export function getDeletedEntryIds() {
  try { return JSON.parse(localStorage.getItem(FEED_DELETED_KEY) || "[]") || []; }
  catch { return []; }
}
function addDeletedEntryId(id) {
  const s = new Set(getDeletedEntryIds());
  s.add(id);
  try { localStorage.setItem(FEED_DELETED_KEY, JSON.stringify([...s])); } catch { /* ignore */ }
}
