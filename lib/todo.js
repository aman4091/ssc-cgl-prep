// 🎯 Weak Topics (pehle "To-do") — homepage par, aur PC ke apne overlay par
// bhi (D:\over\todo_app.py). Dono ek hi list dekhte hain. Andar ka naam
// `todo` / `cgl.todos` jaan-boojh kar wahi rakha hai — badalte to sync aur
// PC overlay ka rishta toot jata.
//
...
// `subject` diya ho to sirf usi subject ke ho chuke hatate hain.
export function clearDone(subject) {
  write(read().filter((t) => !t.done || (subject && subjectOf(t) !== subject)));
}
