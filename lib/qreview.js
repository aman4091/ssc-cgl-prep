...
export function addReview(q, opts = {}) {
  ...
  if (!k || k === "::") return;
  const {
    subject = "", source = "", category = "", chapterId = "", correct = false,
    // Kitna waqt laga is question par (seconds). 0/undefined = pata nahi.
    sec = 0,
    // Dobara-attempt: pehle se maujood record hi sudhro, NAYA mat banao. Ek hi
    // set ko teesri baar dene par galat/chhode question ko notebook mein
    // dobara nahi ghusna chahiye.
    onlyExisting = false,
    // Notebook mein NAYA question sirf PYQ ke set-test se aata hai. Baaki har
    // jagah se (vocab, Current Affairs rush, notes quiz, generated quiz,
    // stylus solve, full-screen runner) sirf pehle se maujood record sudhrta
    // hai — naya nahi banta.
    fromPyq = false,
  } = opts;
  const all = read();
  ...
  // Naya record banega ya nahi — teen shart, teeno poori honi chahiye:
  // PYQ se aaya ho, galat/chhoda hua ho, aur dobara-attempt na ho.
  if (!prev && (onlyExisting || correct || !fromPyq)) return;
  const rec = {
    key: k,
    q,
    ...
    // Aakhri baar kitna waqt laga. 0 aaye to purana rehne do — pata na hone se
    // behtar hai pichhli baar ka aankda.
    sec: sec || prev?.sec || 0,
...
// Fix a stored question's correct answer (e.g. the source had a wrong key). Updates
// the record's answer, re-keys it (key = question::correctText), and marks it
// resolved (correct) so a data-error mistake leaves the Wrong bucket.
export function fixReviewAnswer(key, newAnswer) {
  const all = read();
  const i = all.findIndex((r) => r.key === key);
  if (i < 0) return null;
  const rec = all[i];
  if (!rec.q || !Array.isArray(rec.q.options) || newAnswer == null) return null;
  const q = { ...rec.q, answer: newAnswer };
  all[i] = { ...rec, q, key: keyFor(q), correct: true, at: new Date().toISOString() };
  write(all);
  return all[i];
}
...
export function clearReview(bucket) {
  if (!bucket || bucket === "attempted") { storeRemove(KEY); return; }
