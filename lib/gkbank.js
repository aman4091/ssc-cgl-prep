...
// Supabase sync, so the ~1,700 shared questions would re-upload on every
// unrelated write. Same static-file pattern as public/calcbank and public/cabank
// — fetched on demand, cached in memory for the session.
...
// user with random ids, so the only stable link is the name they gave it — a gs
// chapter called "Polity" picks up the Polity bank.
...
export async function loadGkQuestions(subject) {
  ...
  // In parallel — gs now ships two banks, and fetching them one after the other
  // just makes the PYQ page wait twice.
  const banks = await Promise.all(topics.map((t) => loadGkTopic(t.slug)));
  const out = [];
  topics.forEach((t, i) => {
    for (const q of banks[i]) out.push({ ...q, topic: t.label, gk: true });
  });
  return out;
}
